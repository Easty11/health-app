...
| `feat/hevy-create-loop` | `create_and_resolve` — app-originated custom exercise via create→sync→list-back (DECISIONS_LOG #NEXT, resolves Q14) | **IN FLIGHT** — S2 refactor + S1/S3/tests committed, full suite 38/38 green; governance staged; ff-merge pending | — | Ready to ff-merge to `master` (no external precondition; backend-only, no migration). |
