...
### 256. Sleep stage breakdown re-spec'd: precedence-resolved partition, not independent per-stage unions (supersedes #254's breakdown method)

**Decision.** `_aggregate_day`'s stage breakdown (`backend/routers/health_connect.py`) now derives `deep`/`rem`/`light` from a **single precedence-resolved partition** of the main period's asleep coverage (`_resolve_breakdown`), replacing #254's **three independent per-stage unions** and its multi-source **dominant-source** sub-rule (#254 ruling (1)). Mechanics: walk the stages in precedence order, and for each take its intervals minus everything already claimed by a higher-precedence stage; every wall-clock minute covered by ≥1 asleep segment is assigned to **exactly one** stage. **Precedence order = `[DEEP, REM, LIGHT]`** (operator ruling, Luke — option (i), "deeper/more-specific wins"; the conventional/defensible ordering). Applied **source-agnostically**: a minute two overlapping sessions label differently — within one source (Samsung self-overlap) or across sources — resolves by the same precedence, so the dominant-source pick and its `HC sleep F3a multi-source night …` INFO flag are **retired** with no dependency on F1. **Total-invariant by construction:** AWAKE sits below all asleep stages and is excluded from the partition, so any minute with an asleep segment still resolves asleep — TST stays `_asleep_union_minutes(main)`, the exact value #254 established (verified night: 429, unchanged). New interval helpers `_merge_intervals` / `_interval_seconds` / `_subtract_intervals`; `_union_minutes` refactored onto them, behaviour identical. `_sleep_score` unchanged — still `_sleep_score(deep, rem, tst)`, now fed coherent inputs.

**Coherence is structural, not asserted-and-hoped.** The stage intervals are an exact disjoint partition of the asleep union, so their exact seconds sum to the union's seconds. A **cumulative floor over the precedence order** (floor the running total after each stage, take the delta as that stage's minutes) makes `deep + rem + light == floor(total asleep seconds / 60) == _asleep_union_minutes` **exactly**, regardless of sub-minute segment boundaries — three independent per-stage floors could each shave a sub-minute sliver and leave the parts off the whole, reopening the very incoherence this fixes.

**Rationale.** #254 shipped TST as one union across all asleep intervals but `deep`/`rem`/`light` as three **independent** unions, so a minute overlapping sessions label differently (LIGHT in one, REM in another) was counted once in TST yet in **both** stage buckets → `deep + rem + light > TST`. Verified failing night, wake-date 2026-08-30, **single-source Samsung**, 3 overlapping records: `deep 18 + rem 161 + light 335 = 514` vs `TST 429` — **+85, 120 % of the night**, inflation concentrated in LIGHT (+106 vs Samsung's 229, the most-contested stage across session seams). Root cause is **not** a second source (the dominant branch never fired on a single-source night) and **not** a gap-split (a split only reshuffles which segments land in `main`; it cannot push parts above the whole): it is Samsung writing overlapping records that disagree with themselves at the seams (and re-consolidating differently across syncs — 4 record-starts on one read, 3 on the next). The clean control (2026-08-29, no conflicting overlap) already summed exactly (`27 + 60 + 227 = 314 = TST`), which is why the fault was latent.

**Scope of the claim (recorded, load-bearing).** This fixes **coherence** (parts sum to whole), **not accuracy**. Samsung erased the real ~4 am wake **upstream** (the device under-scores wakefulness); the passive figure stays **reference-only** and titration continues to score off the diary. This fix must not be read as making the passive number clinically trustworthy. **Total permissiveness is explicitly out of scope** (see Q128): the union TST ran 429 vs Samsung's own 402 (+27) because "any session says asleep → asleep" is more permissive than Samsung's merge — a separate question about the total, deliberately not folded in here (this change is total-invariant).

**Status.** BUILT + test-proven; PR open **draft** — harness-originated feature PR, so it merges under the **Q125 lane** (draft, operator-merged; this session does not self-merge). **No schema migration** (computed values only, no columns, no historical backfill — raw sessions are discarded, the recent tail re-aggregates on the rolling window) → not schema-held; the migration-free self-merge property is preserved. Downstream readers unchanged from #254's audit: `mcp_server.py`, `routers/recovery.py`, `routers/checkin_v2.py`, `context_builder.py` each only display the numbers; the CBT-I engine does not read them.

**How you know.** `backend/tests/test_health_connect_sleep_union.py` (9 tests, green): **coherence** — a two-session night with LIGHT 01:00–02:00 + REM 01:30–02:30 (contested 01:30–02:00) asserts `deep+rem+light == tst` and the contested 30 min resolves to REM (precedence over LIGHT) → rem 60 / light 30 / tst 90; **total-invariant** — the stored TST equals `_asleep_union_minutes(main)` over the same segments (the fix does not move the total); **clean-night regression** — a no-conflict night keeps each stage as an independent union would (deep 27 / rem 60 / light 247 / TST 334) and still sums exactly; **single-source self-overlap** — three overlapping Samsung records assert `deep+rem+light == tst` and that resolved LIGHT is **strictly below** the old independent per-stage LIGHT union over the same segments (the +106 inflation is gone); the **multi-source** test was rewritten from dominant-source+log to precedence resolution (Garmin LIGHT absorbed by higher-precedence REM and existing LIGHT → 60/120/240, sums to the 420 union). Full backend suite green (`test_health_connect_sleep*`, `test_hc_sync_*`, `test_readiness_sleep_stages` = 31 passed together; whole suite 1281 passed, 1 skipped), one pre-existing environment-only failure (`test_current_state` `git show 3360ed5:` shallow-clone artifact, unrelated — touches none of these files, fails identically on master). `placeholder guard (POSIX)` clean.

**Do not revisit unless.** The precedence order is re-ruled (options (ii) `[DEEP, LIGHT, REM]` or (iii) `[LIGHT, REM, DEEP]` were on the table; Luke chose (i)) — a re-ruling is a `_STAGE_PRECEDENCE` constant change, not a re-architecture; or F1 lands a real cross-source stage model that supersedes precedence resolution at the seam (precedence is a **labelling** choice, not dedup — it does not reach the total, so F1's total handling via union is untouched); or Q128 (union-total permissiveness, 429 vs Samsung 402) is decided and tightens the total (that is the total, not the breakdown, and would be its own entry). Q126 (`_sleep_score` adequacy term) stays open and reads the same coherent inputs.

---

### 257. Merge-lane unification: one disposition for every Code PR — open-draft→ready→self-merge-in-turn — with a single hold for un-ratified decisions (supersedes the Q125 two-lane convention)

**Decision.** Every Code-authored PR takes one disposition. When the harness opens it as a draft, Code flips it ready-for-review and self-merges the same turn once required checks are green — no operator wait, no confirmation, no scheduled check-in. Session origin (harness or local) creates no operator-merge lane. Holds that do NOT self-merge: (a) a PR containing a schema migration; (b) anything the session was told to hold; (c) a PR embedding a Code decision chat did not ratify — a resolved OPEN_QUESTION, a minted constraint or ordering rule, or a data-meaning default (marker meaning, session-count rule, policy weight). For (c) the decision — not the diff — is surfaced to chat for ratification; the session ends, and a later session lands and closes it out once ratified. Work implementing a chat-approved brief with no new judgment is already ratified and self-merges. Supersedes the "two non-overlapping PR classes" convention resolved under Q125.

**Why.** Q125 recorded a lane where harness-originated PRs are "draft, operator-merged." Reading the harness system prompt directly (2026-08-31): the only real instructions are create-as-draft and, while a PR subscription is live, an hourly check-in until MERGED or CLOSED — there is NO no-self-merge instruction; that half of the Q125 lane was an inference, retracted. Both real instructions discharge in-turn: creating-then-flipping satisfies the draft rule, and merging in-turn meets the subscription's own terminal condition, so no check-in fires. The wait-and-watch loop was self-imposed and contradicted CLAUDE.md's self-merge disposition. Operator ruling (Luke): the only merge-time friction worth its cost is (c) — an un-ratified Code decision — because everything else was already reviewed at design time in chat. Big-but-pre-ratified self-merges; small-but-un-ratified holds.

**Status.** RATIFIED (operator ruling). Convention only — no code, no schema. Lands with the CLAUDE.md § Merge-disposition edit in one governance commit. No harness-config change required for the common path; a held PR ((a)/(b)/(c)) legitimately stays open and ends the turn with no check-in scheduled.

**How you know.** CLAUDE.md § Merge disposition, amended in this commit, carries the draft→ready→self-merge-in-turn sequence, the origin-creates-no-lane clause, and the (c) un-ratified-decision hold. Q125 stays DONE and append-only; this entry supersedes its operative convention, not the question.

**Do not revisit unless.** A non-schema PR class must genuinely not auto-merge for a reason outside (a)/(b)/(c) — name that class explicitly rather than reviving an origin-based lane.

---

### 258. Garmin HRV via server-side garth pull, scoped to withheld metrics only

**Decision.** Garmin withholds HRV, Body Battery, and training readiness from Health Connect by design. Ingest the withheld set via a server-side `python-garminconnect` pull (Garth OAuth+MFA). HRV now; readiness/Body Battery optional later. Garmin sleep/steps/HR stay on the official Garmin→HC feed (`health_connect_syncs`) — resilience: garth is unofficial and breaks on Garmin auth changes, so the commodity signals are NOT moved behind it; a garth outage then costs only HRV. Unofficial + ToS-grey → household/operator accounts only, never B2B/clinical (that lane is a self-fronting aggregator, tracked separately). Credentials: token blob only, never the Garmin password; stored Fernet-encrypted in `UserIntegration(provider="garmin")`, reusing the Hevy pattern (no migration for credentials). Lane = `connectors/garmin.py` (pure connector, no DB) + `routers/garmin.py` (`POST /integrations/garmin/token`, `POST /integrations/garmin/sync`, status/delete) + `scripts/garmin_login.py` (out-of-band interactive login → token blob) + `scripts/garmin_sync.py` (cron/ops runner). A dead/MFA-needed token surfaces as a typed `GarminReconnectError` → 424 "reconnect Garmin", never a 500 (a dead refresh token is expected steady-state). Refresh-token writeback: the possibly-refreshed blob is re-encrypted and committed immediately after login, before the pull, so a refresh is never lost to a later pull failure.

**Rationale.** Confirmed empirically: Deb/user-4's first Garmin→HC sync landed HR+sleep+steps via HC with `hrv_rmssd` null every day and no HRV record type; corroborated by Garmin's published HC data list. The HC path is structurally incapable of carrying Garmin HRV, so the withheld metrics need an out-of-band channel. Concentrating commodity signals (sleep/steps/HR) behind a fragile unofficial auth is the opposite of resilient — use the fragile channel solely for what the official one refuses.

**Adjudications against the live library (garminconnect 0.3.2 / garth 0.8.0, read from source).** The brief assumed `Garmin.get_hrv_data_range(start, end)`; the pinned lib exposes NO range method — only `get_hrv_data(cdate)` — so a range is a per-day loop (`GarminClient.get_hrv_range`). Token store/restore is the inline-JSON `client.loads` / `client.dumps` pair (a blob >512 chars loads inline, no path, no password). MFA login is `Garmin(email, password, return_on_mfa=True).login()` → `("needs_mfa", client_state)` then `resume_login(client_state, code)`. Deps pinned EXACT — a passive bump can silently break the auth/pull lane; **garth is deprecated upstream** (see Q133), which reinforces pinning.

**Status.** BUILT + test-proven (16 tests, `backend/tests/test_garmin_hrv.py`). Schema migration `b7c3e9d15a20` for the storage pair (#259) is **HELD** for explicit operator release; the ingestion code lands with it but the migration is not applied. Consumption (recovery.py rewire, Samsung migration) is out of scope (Q130). PR holds under #257 (a) — contains a schema migration.

**How you know.** `backend/tests/test_garmin_hrv.py` green (16): canned `/hrv-service/hrv/{date}` payload → normalised rows vs hand-computed oracle; per-day range loop drops empty days; bounds (RMSSD 1–400) null the nightly avg / skip a bad sample and keep siblings; idempotent re-sync overwrites the parent (same id, no dup) and replaces children; refresh token re-encrypted back to `UserIntegration`; dead token → `GarminReconnectError` and the sync endpoint → 424; token stored encrypted (not plaintext). Library mechanism verified against installed source (`get_hrv_data` signature, `client.dumps/loads`, `login(return_on_mfa)`/`resume_login`).

**Do not revisit unless.** Garmin publishes HRV to Health Connect (then the fragile lane is retired for the official one); or the B2B/clinical path opens (a self-fronting aggregator, never this ToS-grey pull); or garth's deprecation (Q133) forces a maintained fork / different transport.

---

### 259. HRV storage is a source-agnostic pair, not a generalisation of the Samsung table

**Decision.** HRV storage is a new source-agnostic pair, NOT a generalisation of `samsung_hrv_readings`. `hrv_readings` — nightly, `source` column, `rmssd_ms` nightly average + nullable Garmin-richer fields (`status`, `baseline_low`, `baseline_high`, `weekly_avg`); unique `(user_id, captured_at, source)`. `hrv_samples` — the 5-min RMSSD series, FK to the parent (ON DELETE CASCADE), unique `(hrv_reading_id, reading_time)`; Garmin populates it, nightly-only sources leave it empty. HRV cross-source arbitration added as `reads/recovery_reads.py` mirroring `reads/aerobic_reads.py`: a derived, never-persisted `.canonical` (no `canonical` column), `_SOURCE_RANK = {garmin: 2, samsung: 1}`, grouped by the night `(user_id, captured_at)` — simpler than aerobic's interval-overlap because a night is a night.

**Rationale.** `samsung_hrv_readings` conflates HRV with Samsung sleep architecture (rem/deep/light minutes, efficiency, spo2, bedtime…). Generalising it would drag sleep columns into an HRV abstraction Garmin barely uses, while Garmin's own sleep lives in `health_connect_syncs`. A clean HRV-only pair keeps provenance and shape honest. Read-time arbitration is order-independent and reversible, the same reasoning as aerobic (#255-era `aerobic_reads`): the winner must not depend on sync arrival order.

**Status.** BUILT. Migration `b7c3e9d15a20` (additive only, no data move) is **HELD** for operator release. `recovery_reads` is ready-but-single-source now (Garmin only) — exactly as aerobic arbitration predated all its sources landing. Samsung HRV migration into `hrv_readings` + the `recovery.py` HRV-read rewire is the deferred consumption follow-on (Q130). SCHEMA.md updated in the same change (tables 029/030).

**How you know.** `backend/tests/test_garmin_hrv.py` arbitration tests: single-source night canonical by default; contested night → exactly one canonical, Garmin rank wins; separate nights each canonical; `canonical_hrv` windows by `since` and flags. Model classes + migration reviewed against `models.SamsungHRVReading` / `models.AerobicSession` shapes; the pair carries no sleep columns.

**Do not revisit unless.** A single user acquires both Garmin and Samsung HRV and the rank needs re-tuning (Q131); or the Samsung-unification follow-on (Q130) changes the store's shape; or a persisted-canonical decision supersedes read-time arbitration (a separate decision, as it is for aerobic).

---

### 260. Metabolic transform sources rows through same-bout arbitration; flow_export outranks v4

**Decision.** `load_events_metabolic.compute_metabolic_load_events` now sources its rows through `reads/aerobic_reads.arbitrated_sessions` (full-user set, `.canonical` set) instead of a raw `select(AerobicSession)`, and emits a `load_event` only from a row that is BOTH canonical (the winning row of its same-bout cluster) AND qualifying (INV-7). Single-emission of the metabolic series is now a guaranteed property, not the accidental side effect of the fail-closed skip landing on a zoneless v4 twin (Q127). As part of the same change, `_SOURCE_RANK` was corrected from `polar_v4 == polar_flow_export` (both 2) to `polar_flow_export` (3) > `polar_v4` (2) > `health_connect` (1). `metab-v1` is unchanged (no `formula_version` bump); no schema; delete-then-insert idempotent recompute unchanged.

**Rationale.** The brief's no-op guarantee ("the zoneless v4 twin is non-canonical tomorrow; the same zoned row emits") assumed `flow_export` outranks its `v4` twin. It did not: the two rows parse via the same `import_polar._parse_session` (byte-for-byte parity — `connectors/polar.PolarV4Client.parse_session`), so a same-bout twin has an identical `[start, stop]` interval and, with rank tied, `_win_key` fell through duration→start to the `id` tie-break. Twins coexist exactly when a v4 sync precedes a Flow-export backfill (the documented remediation path for stale zoneless v4 sessions; `routers/polar` dedups v4 against both lanes but `import_polar` does not dedup the export against the v4 lane), and in that order the earlier-ingested v4 row holds the lower id → won canonical → was skipped fail-closed (zoneless) → the bout dropped entirely. The landed `test_tie_equal_duration_earliest_start_wins` already encoded a v4 row beating a flow_export row. So the literal brief was a data-losing regression, not a no-op. The v4 *list* transport omits trainingLoadReport/zones (cardio_load/muscle_load/z*_seconds null), so for a same-bout twin the Flow-export row is STRICTLY richer; ranking it above v4 makes it deterministically canonical, fixes the root cause for every arbitration consumer (recovery reads / UI see the richer row), and makes the brief's emit predicate correct as written. Chosen over a transform-only "canonical-among-qualifying" reinterpretation because that left the id nondeterminism the brief's Piece 2 (v4 zone-enrichment) depends against.

**Status.** BUILT + test-proven, non-schema, self-merged in-turn (pre-ratified brief; the one new judgment — the rank change beyond the brief's transform-only scope — was operator-ratified before landing). Consumption cascade (`load_metrics` + Banister rollup) unchanged; it lights up from the same `metab-v1` rows.

**How you know.** `backend/tests/test_load_events_metabolic.py`: `test_v4_flow_export_twin_is_noop_zoned_row_emits` (v4 at the lower id — the regression order — still yields one row from the flow_export twin, v4 counted in `sessions_skipped_non_canonical`), `test_zoned_cross_source_overlap_collapses_to_one_emission` (two zoned twins → one emission), `test_non_overlapping_zoned_sessions_both_emit` (distinct bouts untouched); `backend/tests/test_aerobic_arbitration.py`: `test_flow_export_outranks_v4_twin_identical_interval`, `test_v4_outranks_health_connect_twin`, tie-break tiers re-exercised with two unknown-rank sources. Full backend suite green (1302 passed) save the pre-existing `test_current_state.py` snapshot test, which shells `git show 3360ed5:…` against a commit object absent from this clone (environmental, unrelated to this diff).

**Do not revisit unless.** A future source legitimately makes `polar_v4` richer than `polar_flow_export` for a shared bout (would invert the rank); or a persisted-canonical decision supersedes read-time arbitration (a separate decision, as noted for aerobic and HRV); or Piece 2 (v4 zone-enrichment, the `metab-v1`→`v2` fork, Q123/#255) changes what "same-bout cluster" means.

---

### 261. Polar v4 sync folds in per-exercise zone enrichment (feature mode), populating `z*_seconds` on zoneless `polar_v4` rows; `metab-v1` retained

**Decision.** `POST /integrations/polar/sync` now runs a second pass after the summary list/upsert: for exactly the dates that carry a zoneless `polar_v4` row in the sync window, it fetches the per-exercise HR-zone split via v4 **feature mode** and merges `z*_seconds` onto those rows in place (`routers/polar._enrich_v4_zones`; `connectors/polar.PolarV4Client.list_zoned_sessions`). The on-ingest metabolic cascade then recomputes — previously-zoneless bouts now qualify (INV-7) and emit. **No formula bump — `metab-v1` retained** (the brief's ruling): enrichment adds *transport*, not a new scoring rule, so the `metab-v1`→`v2` zone-less-mapping fork (Q123/#255) stays a separate future option, NOT triggered here. `provenance.zone_source` already distinguishes a v4-enriched row (`polar_v4`) from a ZIP row (`polar_flow_export`) per-row, so the two zone paths remain legible post-recompute. No schema; `metab-v1` delete-then-insert recompute unchanged.

**Feasibility — live-probe-confirmed (the gate the build stood behind).** The v4 `training-sessions/list` **summary** transport omits the zone split (#255/Q123), but the API exposes it under a `features` query parameter. The exact contract was established by a read-only in-container probe against the live v4 API (the swagger host is egress-blocked from the build sandbox, so the probe was the gate — not an assumption; unseeable-surface rule): (a) the token is **lowercase `features=zones`** — case-sensitive; an unrecognised value (e.g. `ZONES`) is silently ignored (HTTP 200, no zones), which had earlier read as a false "accepted"; (b) feature mode **caps `to - from` at ONE day** (`400 "must not exceed 1 day(s) when feature parameter is given"`) → `list_zoned_sessions` loops per-day; (c) the returned zone schema is **byte-identical to the Flow ZIP export** — `exercises[].zones[ZONE_TYPE_HEART_RATE].zones[].inZone` (ms) — so `import_polar._parse_session` reads it verbatim (probe ran the real parser: `inZone` 1235000/1665000/… → `z*_seconds` 1235/1665/…). Rate limits (3000/15min, 100k/24h) are untroubled: the fetch is scoped to session-days, one call each.

**Design choices (within the pre-ratified brief; no new judgment minted).** (1) The per-day fetch is scoped to the dates of zoneless rows, not every day in the window — same mechanism, strictly fewer calls; a sync whose v4 rows are already zoned makes zero extra calls, and the default 365-day window never fans out to 365 requests. (2) "Zoneless" is the transform's own INV-7 predicate (NOT qualifying — zone-sum 0), because a summary-synced v4 row stores `z*_seconds = 0` (the column default), never NULL — so an `IS NULL` filter would have matched nothing. (3) A row is overwritten only when its fetched split is itself qualifying, making re-syncs idempotent (an enriched row is no longer a target). (4) Cross-source dedup means a v4 twin only exists when v4 synced BEFORE the Flow-export import; enriching that twin is safe because `flow_export` outranks `v4` (#260), so the richer row stays canonical and the bout still emits once.

**Scope (fork-2 consequence — a deliberate one-time backfill, not automatic).** Folding into the sync enriches the **sync window only**. Historical zoneless v4 rows outside recent windows stay zoneless until touched; to backfill, run one wide-window sync (large `days`) after this lands — same mechanism, bigger range — then ongoing syncs keep it current.

**Ordering gate (Q127 verbatim, load-bearing).** Enrichment must not precede #260's arbitration in the code path, else every dual-lane bout double-emits. Satisfied: #260 (PR #143) is on master and this branch is based on it, so the arbitration routing ships with (and ahead of) this change. The merge additionally holds for operator confirmation that #260's prod recompute has run (an unseeable surface this session cannot verify).

**Status.** LANDED — merged via PR #144 (merge `a936e429`), branch remote-deleted. Both holds DISCHARGED: human review taken (operator-authorised merge), and **#260's prod recompute is CONFIRMED RUN** — the operator ran `load_events_metabolic.py` against prod: **47** metabolic `load_events` written, **17** non-canonical skips (Q127 arbitration collapsing dual-lane twins — real overlap, not idle), **3** zoneless, and **r=0.974** between Edwards TRIMP and Polar's independent `cardio_load` (n=46). The correlation is the proof the recompute produced sane numbers, not merely ran without erroring. The Q127 ordering gate reads DONE, not pending-operator — a future session must not re-open the hold. metab-v1 unchanged; non-schema.

**How you know.** `backend/tests/test_polar_zone_enrichment.py`: `test_list_zoned_sessions_uses_features_zones_and_one_day_windows` (transport-layer fake per #166 — asserts the real request carries `features=zones` + a one-day window and the real parser yields the zone seconds), `test_list_zoned_sessions_scopes_one_call_per_day`, `test_sync_enriches_v4_only_bout_and_emits_exactly_once` (v4-only bout gains zones → one metabolic event, `zone_source=polar_v4`), `test_sync_second_run_reenriches_nothing` (idempotent), `test_enriched_v4_twin_stays_non_canonical_behind_flow_export` (dual-lane: enriched v4 twin counted in `sessions_skipped_non_canonical`, the single emission stays `zone_source=polar_flow_export`). Full backend suite green (1307 passed, 1 skipped) save the pre-existing `test_current_state.py` snapshot (shells `git show 3360ed5:` against a commit absent from this shallow clone — environmental, unrelated, fails identically on master). `placeholder guard (POSIX)` clean.
...
### 275. v1 definition of done adopted; surfacing-phase sequence set; co-equal-modules principle amended to sequence-by-consumer

**Decision.** (1) **v1 is a demonstrable personal proof case** for one real user (Luke), defined by four tests — **See** (load/readiness trends, visual, no chat), **Know** (what's due, enforced against the plan), **Walk in** (auto-generated pre-appointment brief synthesising labs, interpretation, injuries, protocol, training state), **Loop** (check-in → readiness → recommendation → log → close-out as a daily habit, chat not required). B2B (multi-tenant, clinician view, patient-outcome reporting) is explicitly OUT of v1 — a separate build that starts only after v1 is demonstrable. (2) **Surfacing-phase sequence:** weekly resolver (test 2, Oct 5 anchor) → visuals lane done-data-first (test 1; wrap-vs-plan after the resolver) → appointment brief v1 (test 3) → surface-debt sweep (test 4). (3) **Co-equal-modules principle amended:** the three modules (Fitness, Medical Protocol, Decision Support) remain co-equal *in value*, but build order is *sequenced by the synthesising consumer* (the appointment brief), not by lane momentum — anything not on the path to a v1 test is LATER by definition. This refines the `FEEDBACK` §1 "three co-equal modules" principle: it adds the sequencing rule; the co-equal-in-value claim stands. (4) **Demotions to LATER** on the "not on v1 path" rationale: Exposure UI increment 3 (dose / the Q106 `minutes`→dose reading; the resolver itself stays in NEXT), the microcycle editor, and CBT-I module phase 2.

**Rationale.** The machinery (triage, close-out, sprint selection) had no committed destination to triage against, so "what's next" drifted by lane momentum — three consecutive sessions went to UI/CI instrument rather than to the health-intelligence core (the #274 close-out named this). Writing the v1 definition, the consumer-led sequencing, and the surfacing sequence into the tree gives the loop a fixed target; the demotions remove work momentum had kept near-term that no v1 test needs.

**Status.** ADOPTED + RATIFIED (2026-09-09; governance-only: ROADMAP + `closeout.md` + the `/closeout` ritual + this entry). The demotion list was **confirmed as-is** by the operator; the two concerns with no standalone ROADMAP row — the Banister dosing wire (`selection.py` ← `load_metrics`) and further psychological-window work (Q122) — stay where they sit by that ruling (a Q may not be relocated), not minted as new rows. The unverified "Georgia on Polar H10" rollout belief was **operator-confirmed dropped** — not a platform user; Q98's three-account roster (Luke/Deb/Cooper) stands. Self-merged on green after in-session ratification (the v1 definition and the amended principle were new strategic judgments; the hold was the ratification, now discharged).

**How you know.** `ROADMAP.md` now opens with `## v1 — definition of done` and `## Surfacing phase — sequence` before NOW; the Appointment brief NOW row carries "sequence position 3"; the three demoted rows sit in LATER with a `DEMOTED (2026-09-09)` stamp and their notes preserved; LATER, User rollout, and Dependencies/blockers were rewritten with per-row citations (User rollout is the operator-confirmed three-account roster — Luke/Deb/Cooper, Q98; the "Georgia on Polar H10" belief was dropped as unsupported); `closeout.md` and `.claude/commands/closeout.md` gained the "which v1 test does this lane serve?" handoff prompt. All on `claude/roadmap-v1-steer-i5l54q`.

**Do not revisit unless.** v1 is demonstrable (then B2B is admitted and the sequencing principle re-opens for the multi-tenant build); or a v1 test's own definition is amended.

---

### 276. Due-slot resolver — unified quota window over phase microcycle / weekly_template, `/engine/next` defaults to the due slot

**Decision.** A new `engine/resolver.py` resolves *which slot is due* — the consumer Q106/`#221` deferred and `#270` scoped. A **quota window** is a dated span carrying ordered `{capacity, quota}` slots, produced by one of two in precedence: an open `training_phase` WITH a `microcycle` → the current A/B leg (quota = `sessions_per_cycle`); else a `fortification_profile` → this Mon–Sun week (quota = `sessions_per_week`); neither → None. Completion is derived on read from Hevy, never written back. Five design calls:

- **Leg indexing** = `floor((today − entered_on) / sub_cycle_days) mod len(sub_cycles)`; the leg's window is its own `[start, end]` in whole sub-cycle lengths; label from the sub-cycle's own `label`, else A/B/C/D by order.
- **Local-day indexing (the trap):** every date — `today` and each workout's UTC `start_time` — is bucketed to a Brisbane day via the single-source `_local_day` (AEST, no DST) before any arithmetic; a UTC `::date` bucket mis-dates a late-UTC session onto the wrong day.
- **Rule 1 — dominant capacity:** the capacity with the most PRIMARY-tagged exercises in the workout (`exercise_region_tags.role == 'primary'` → `Region.capacity`); secondary tags are ignored — a deliberate departure from `infer_loaded_regions`, which answers coverage, not "what was this session". Tie → the tied capacity first in the window's slot order. One workout, one slot.
- **Rule 4 — due slot:** the first slot in DECLARED order with `remaining > 0`; None if every slot is met. No fraction-remaining weighting — declared order is what the ordering is for.
- **`uncounted[]` surfaces, never swallows:** a zero-primary-tag workout is `reason: "untagged"` (+ its untagged-exercise count); a workout whose dominant capacity is not a slot is `reason: "off_plan"` (+ that capacity). Dedup/excluded workouts are filtered out entirely.

`GET /engine/resolver` returns `{window, slots[], due_capacity, uncounted}` (null window, 200, at baseline — the `#272` no-profile contract). **`GET /engine/next` defaults to the due slot** when `capacity=` is omitted; an explicit `capacity=` still wins unchanged, and a None due-capacity (baseline, or every slot met) leaves `/next` unfiltered. `minutes` is never read — Q106 stays open on `minutes` alone; Q105 resolve-before-compare is honoured on every slot key. No schema, no scheduler, no calendar, no write-back; the suppressed-probe path (`#271`) is untouched.

**Rationale.** `#221` built `weekly_template` as an inert declaration and `#270` gave the resolver a phase-scoped input, both deferring the consumer. v1 test 2 (Know — "what's due, enforced against the plan") needs it, so the v1 steer (`#275`) sequenced it first with an Oct 5 anchor. Rules 1 and 4 were operator-ratified this session — the brief referenced them as "rule 1 / rule 4" with no in-tree definition.

**Supersedes** `#221`'s router-level statement that "storing a template must not change unconstrained `/next`" — which held only while the consumer was unbuilt; `/next` now defaults to the due slot. The engine-level invariant stands: `select_next` is unchanged and enforcement lives in the router, so `#221`'s gate-f engine test (`with_tpl == without` over `select_next`) is unaffected.

**Status.** BUILT + test-proven; non-schema; self-merges on green (chat-approved brief, rules 1/4 ratified in-session — no un-ratified judgment). Panel wiring (the brief's STEP 7) is split to its own follow-up PR: it is frontend, unseeable from chat, and its GATE is a served-bundle probe (`#121`).

**How you know.** `backend/tests/test_resolver.py` (16 green): leg boundary day-before/after and wrap; the 23:30-UTC → next-Brisbane-day trap; the dominant tie broken by slot order AND re-broken when the order is flipped; a secondary-only + zero-tag workout surfaced as `untagged` (proving secondary is ignored); an off-plan workout surfaced distinctly; dedup/excluded not counted; weekly fallback with an UPPERCASE token (Q105); phase-over-weekly precedence and phase-without-microcycle → weekly; Rule 4 first-unmet-in-order and None-when-all-met; baseline null window; and over HTTP `/engine/next` default-to-due, explicit-wins, unfiltered-when-met, plus `/engine/resolver` and its baseline null. Full backend suite 1444 passed (the 4 red are the pre-existing Q137 AEST-vs-`date.today()` skew and the `#260` shallow-clone `git show` test — all red on master too, none in this diff's files). `grep minutes engine/resolver.py` → empty.

**Do not revisit unless.** Q106 settles `minutes` (a slot's dose then has a reading the resolver or dosing seam consumes); or a scheduler/calendar replaces week-to-date derivation; or `_local_day`'s UTC-instant assumption for `start_time` is falsified (then the bucket rule changes, per the load_metrics release-gate).

---

### 277. Recharts adopted as the charting library; `/series` is the chart read surface

**Decision.** (1) **Recharts** is the app's charting library (Visuals increment 1, v1 test 1 "See"). It renders to SVG — assertable in jsdom at a fixed size — so the commodity rendering is bought while the data shaping stays owned in our code. (2) **`routers/series.py`, mounted at `/series`, is the one chart read surface** for every chart the surfacing phase draws; `/series/readiness` and `/series/exercise/{id}` land here later and reuse the same shape. (3) First route `GET /series/load?days=&windows=` returns `{days, metrics_version, windows: [{load_window, unit, formula_version, points: [{day, daily_load, fitness, fatigue, form, acute_load, chronic_load, maturity}]}]}` for the current user, ascending by day, over the trailing `days` window (default 90). (4) Every chart mounts through one wrapper, `components/charts/TimeSeriesChart` — **fixed width/height by default, `ResponsiveContainer` only opt-in** — because a responsive container is 0×0 in jsdom and would render no series to assert on. (5) `frontend` `engines.node` bumped `>=20` → `>=22`, aligning `package.json` with the Node-22 test stack the CI test lane (#273) already requires (jsdom@30/undici@8 need `worker_threads.markAsUncloneable`, Node 22+).

**Version pinning (the reason `/series/load` is not a naive `SELECT *`).** `metrics_version` is one axis — `banister-v1`, imported from `load_metrics.METRICS_VERSION` so the read surface can never drift from the τ-set the rollup wrote; rows at any retired metrics_version are excluded. `formula_version` is **per-lane, not global** — `tier0-v1` for mechanical/neuromuscular, `metab-v1` for metabolic (`load_events_metabolic.FORMULA_VERSION_METABOLIC`), psychological fail-closed (Q122) — so there is no single constant to filter on (mcp_server pins only its own metabolic lane, by literal). The endpoint therefore filters metrics_version and, per window, keeps a SINGLE formula_version (the most recently computed), which under the delete-and-reinsert recompute (D-B) is the whole of a window's series today and stays unambiguous if a superseded formula_version ever lingers beside the current one.

**Rationale.** The surfacing phase (#275) needs the first real chart, and a chart library plus a read surface are foundational choices the next two increments inherit — so they are decided once, here, with the cheapest increment (work-done load, one endpoint, one line per window). SVG-over-canvas was chosen precisely so the GATE "assert on rendered SVG, never on it-didn't-throw" is meetable.

**Status.** BUILT + test-proven; **read-only, non-schema, no migration** (backend reads `load_metrics` only — no change to `load_metrics.py` computation or the MCP tool); self-merges on green (chat-approved brief, no new un-ratified judgment). The #121 served-bundle grep is a post-deploy step handed to the operator (this session has no prod egress); the populated-window set for user 1 could not be read from the sandbox (no prod-Postgres route here) and is likewise handed over — the endpoint returns whatever is populated, hardcoding no set.

**How you know.** `backend/tests/test_series_load.py` (9 green): response shape, ascending-by-day, default-all-windows, `windows` allowlist, unknown-window-is-empty-not-422, `days` bound, user scoping, only-current-metrics_version, and per-window collapse-to-latest-formula_version. `frontend/src/components/charts/LoadChart.test.jsx` (6 green, jsdom): one line per window, one legend entry per window, one dot per point in one dot-group per window, the maturity (cold-start) styling boundary present and both-sided, the empty state (no SVG), and the range selector reissuing the request with the chosen `days`. Full suites green in a clean env: backend 1456 passed (the one red, `test_current_state`, is the pre-existing shallow-clone `git show 3360ed5` — red on master too, `fetch-depth: 0` in CI); frontend 124 passed. Bundle-size delta measured by build-with vs build-without: **+369.9 kB raw / +107.4 kB gzip** (essentially Recharts).

**Readiness-history note (VERIFY, for increment 2).** A route returning readiness over time already exists: `checkin_v2 GET /history` → `list[DailyRecordOut]`, carrying `date`, `morning_readiness`, `model_forecast`, `model_confidence`, `passive_hrv_ms` (default 14 days, newest first). Increment 2's `/series/readiness` shapes from that store rather than minting a new one.

**Do not revisit unless.** Recharts stops rendering assertable SVG or drops React support (then the library choice re-opens, the `/series` shape does not); or a window ever legitimately needs two formula_versions shown at once (then the per-window collapse rule changes); or `load_metrics` gains a second live metrics_version (then the single-axis pin needs a selector).

---

### 278. Exercise progression + phase markers (Visuals increment 3) — per-template e1RM/volume on `/metrics`, training-phase boundaries overlaid on every chart

**Decision.** Five design calls, all from the chat-approved increment-3 brief (D1–D5):

- **D1 — Metric: e1RM by Epley, per session, from the top set.** `e1rm = weight × (1 + reps/30)` over each working set; the session value is the **max**. Epley degrades above ~10 reps, so a set with **reps > 10 does NOT contribute to e1RM** — but it **does** count toward volume. This is the strength signal (what Hevy shows, so what Luke sanity-checks against).
- **D2 — Volume = Σ(weight_kg × reps)** over the session's working sets, `kg·reps`. A second small multiple, **bars**, on its own axis — a different unit from e1RM, so the #186 shared-axis⇒shared-unit guard would refuse an overlay anyway. This is the work signal.
- **D3 — Working sets only.** A set typed `warmup` is excluded; `normal`/`failure`/`dropset` (and an untyped set, which the `load_events` transform reads as `normal`) are included. Workouts with `excluded_at` set OR `dedup_flag` true are excluded — **the same filter `engine/resolver` applies** (`excluded_at IS NULL` AND `dedup_flag IS NOT TRUE`), so this read surface and the load path see one population.
- **D4 — Phase markers on every `/metrics` chart.** A `ReferenceLine` at each `training_phases` `entered_on` and `closed_on` in range, labelled with the phase name; baseline (no phase) needs no marker. One shared component (`PhaseMarkers`), one fetch of the phase ledger per page. Because a Recharts x-axis here is categorical, each boundary is **snapped to the first chart category on or after it** (the first day the new phase's effect is visible); a boundary before the window's first category is dropped (off-screen, "in range" only), and boundaries that snap to the same category collapse to one line.
- **D5 — Selector** lists templates with **≥ 3 qualifying session-days** in range, most-frequent first, defaulting to the most frequent. A session-day qualifies for a template when it carries ≥ 1 working set — the same qualification that makes a point in `/series/exercise/{id}`, so `sessions` equals the number of points that route returns.

**The phase ledger is reused, not re-endpointed.** `GET /engine/phase/history` already returns every phase with `entered_on`, `closed_on` and `label`, so the brief's "don't add a route that duplicates one" is honoured — there is **no `/series/phases`**.

**Rationale.** This extends v1 test 1 (See) and is the first chart Hevy cannot draw: the phase boundary overlaid on the e1RM line — "e1RM fell here, that's when Decompression started" — is the platform's read, not a fitness-app clone. e1RM and volume are computed at read time from `hevy_workouts.raw` (the recompute source of truth) because no per-template rollup exists; it mints no data-meaning the load path doesn't already own (Epley is the standard open e1RM estimator; volume is kg·reps as logged).

**Status.** BUILT + test-proven; **read-only, non-schema, no migration** (no change to Hevy sync, dedup, region tags, or `training_phases`); self-merged on green (chat-approved brief, no new un-ratified judgment — the reuse of `/engine/phase/history` and the snapping rule are implementation details the brief invited). No RPE (Hevy's is optional/sparse — a later increment); no cross-exercise comparison.

**How you know.** `backend/tests/test_series_exercise.py` (14 green): Epley on a known set + the session max; reps>cap excluded from e1RM but counted in volume; e1RM null when every set is over cap (volume still set); warmup excluded everywhere; a warmup-only session is no point; dedup + excluded workouts contribute nothing; two same-day workouts collapse to one point (volume summed, e1RM max); ascending order; selector ≥3 threshold + desc-by-sessions ordering; warmup-only day doesn't count; missing-template title falls back to the id; user scoping; days bound. `frontend` (21 new vitest): `PhaseMarkers` (`phaseBoundaries`, `snapToCategory` before/after-range drop, `referenceLinesFor` snap + collapse + out-of-range); `TimeSeriesChart` markers case (reference line at an in-range date, label present, out-of-range none); `ExerciseChart` (two panels on distinct units kg vs kg·reps, a gap where e1RM is null, markers reach the chart, selector most-frequent-first + default, re-fetch on pick, empty state); and `PhaseMarkersOnCharts` — a `ReferenceLine` on **all four** charts (Load, Form, Readiness, Exercise). Full frontend suite 158 passed. New eslint: 0. Bundle delta build-with vs build-without: **+8.79 kB raw / +2.09 kB gzip**. The #121 served-bundle grep (`e1RM` + the `phase-marker` label — both confirmed present in the built bundle here) is the operator's post-deploy step (no prod egress this session).

**Increment-2 operator OWEDs closed (operator-confirmed this session).** The two #277/increment-2 hand-offs are discharged: the #121 served-bundle grep passed on `index-BSgJsRq2.js` (Fitness/Readiness render strings + the unit-guard message served), and the populated-window read is **mechanical, metabolic, neuromuscular**.

**Do not revisit unless.** RPE becomes dense enough to chart (a later increment); a per-exercise derived rollup is introduced (then the read-time computation moves); or the Epley estimator / rep cap is replaced (then D1 re-opens).

### 279. Lab visuals — `/series/lab*` marker series + stepped-reference-band trend on `/labs`

**Decision.** Six design calls, all from the chat-approved Lab-visuals brief (D1–D6). The reusable piece is the endpoint; the chart is its first consumer, and the appointment brief (v1 test 3) reads the same `/series/lab*` surface.

- **D1 — Per-point range, stepped band; colour from the platform's verdict.** Each point carries its own `ref_low`/`ref_high` + exclusive flags from its row; the band between two consecutive draws takes the **earlier** draw's range (N draws ⇒ N−1 `ReferenceArea` segments). Point colour is the stored `computed_flag` (H/L), **never re-derived** — the extractor computed it per labs.py's documented rule (which honours the exclusive flags: `>=`/`<=`) and it is stored verbatim. The tooltip shows `lab_flag` alongside when the two **disagree** — an edge value or an OCR slip, a real signal, not noise.
- **D2 — Censored values are hollow markers at the bound, never line vertices.** A row with `value_operator` (`<`/`>`) draws an open `ReferenceDot` at `value_num` and the trend line **skips it** (null in the line data ⇒ a gap). Plotting `<0.3` as a measured `0.3` fabricates a measurement the assay never made.
- **D3 — Excluded, not dropped.** Qualitative results (no numeric value), unmapped rows (a historical draw of the marker's raw name never promoted to the canonical), and results whose `unit_canonical ≠ unit_established` do not plot; the endpoint returns them in `excluded[]` with a reason (`qualitative` / `unit_mismatch` / `unmapped`) and the page shows counts, unmapped ones pointing at the bind control on the ingestion surface below. **No unit conversion** — a mismatch is surfaced, never converted (a chart that silently converts is a chart that lies about what the lab measured).
- **D4 — Time axis, additive.** `TimeSeriesChart` gains `xType` (`'category'` default, unchanged; `'time'` = numeric epoch-day x with date ticks). Labs use `'time'` so irregular draw spacing reads truthfully (a 90-day and a 365-day gap render proportionally); training charts stay categorical. A `children` slot was added the same way so a caller can pass `ReferenceArea`/`ReferenceDot` as direct children (Recharts only registers those it finds directly), and a `tooltipContent` slot for the custom tooltip.
- **D5 — Derived and low-confidence marked.** `is_derived` → a distinct marker **shape** (diamond, legible without colour) + a tooltip note; `confidence < 0.85` → muted (opacity + dashed stroke), the same treatment as a cold-start bar. `0.85` is the **one** extraction threshold in the codebase (labs.py's `field_confidence.* < 0.85` suspect rule); the stored per-row `confidence` is the **min** of a row's field confidences, so `confidence < 0.85` on the stored value reproduces the confirmation screen's low-confidence rule exactly.
- **D6 — No panel grouping in v1.** Marker picker ordered by most-recent draw, with a "flagged" filter (any out-of-range point). Panel grouping needs a field the marker map does not carry — a **LATER** row on ROADMAP, not a Q.

**Rationale.** Serves v1 test 3 (Walk in) as a prerequisite: the appointment brief synthesises across modules and needs a per-marker series it can read, so the endpoint is the deliverable and the chart proves it. It mints no data-meaning — every value, range, flag, and confidence is relayed from `lab_results` as the platform already recorded it; the chart's only judgments are presentational (which encoding, which shape), and those follow FEEDBACK §40's rule that a rendering choice is a data-meaning claim (censored ≠ measured; a stepped band ≠ one flat range; mismatched units are not one series).

**Status.** BUILT + test-proven; **read-only, non-schema, no migration** (no change to extraction, canonicalisation, binding, or `computed_flag`; same auth as `/labs/results`, nothing new exposed). Self-merged on green — a chat-approved brief with no new un-ratified judgment (the VERIFY items all confirmed against the model; the confidence-threshold clarification is a reading of existing behaviour, not a new call). Q141 untouched.

**How you know.** `backend/tests/test_lab_series.py` (10 green): points ascending with per-point ranges (the two labs' 10–30 vs 8–28 both carried); the censored point carries `operator`/bound and every other point's operator is null; the exclusive-bound verdict (28 at an exclusive upper bound ⇒ H, in-range if inclusive) carried from `computed_flag`, not re-derived; derived and low-confidence points marked not dropped; the three exclusion reasons keyed to the right reports and disjoint from the plotted dates; `from`/`to` bounding; detail and index user scoping; index newest-first + `any_flagged` + `display_name` falling back to canonical (unwired on the seed); unknown marker 404. `frontend` (15 new vitest): `TimeSeriesChart` time-axis proportion (90d vs 365d at 90/365 of the span, not the 0.5 a category axis gives); `LabChart` band segments = draws−1, one hollow reference-dot for the censored value + a measured dot for every other point (the gap), derived/muted counts, excluded summary by reason with the unmapped bind hint, empty state; `LabsPanel`/`LabsPicker` newest-first list, flagged filter, select-fetches-series, empty state. Full frontend suite 174 passed; new eslint 0. VERIFY at build: the marker map seeds 70 rows, `unit_established` null on 4 (egfr, haemolysis_index, haematocrit, chol_hdl_ratio — the unitless ratios), `display_name` null on all 70. The #121 served-bundle grep (the neutral string `Reference range`, confirmed present in the built bundle here) is the operator's post-deploy step (no prod egress this session).

**Do not revisit unless.** The marker map gains a panel/system field (then D6's LATER row becomes buildable); a per-marker derived rollup is introduced (then the read-time series moves); `computed_flag` is found NOT to honour the exclusive flags (a canonicalisation finding — then D1's "don't re-derive" re-opens, and it is FEEDBACK-logged there, not here); or unit conversion is ever wanted in the chart (it is deliberately refused — D3).

### 280. Lab batch 2026-08-04 + 2026-09-02 — ingestion adjudicated against master; three canonical binds ratified; imaging deferred

**Decision.** The 24-report / 97-result SNP batch (handoff `INGESTION_HANDOFF.md`; extract stage done off-repo, self-verified) was **adjudicated against master, not stored by this session**. Five reconciliations + two ratified operator calls:

- **Table binding reconciled.** Write target is `lab_reports` (1/report) + `lab_results` (N children); `collected_date` is a **DATE**, not a datetime — the handoff's "anchor each on its own collected datetime" cannot hold at time granularity (both 09-02 events store `2026-09-02`). `POST /labs/confirm` resolves raw→canonical **from the live `marker_canonical_entries` map by `marker_name_raw`, ignoring the extractor's proposed `marker_canonical`** — every proposed key in the JSON is advisory only.
- **Two 09-02 events stay separate by construction.** `544974970` (Rangaswamaiah, endocrine — 9 report envelopes) and `544975535` (Wolanski, PSA + Urine MCS — 2 envelopes) are distinct `lab_report` rows; they share only `collected_date` and carry **disjoint marker sets**, so the `(user, marker, collected_date)` series-integrity guard never false-trips across them. No merge handling needed — they are already separate envelopes.
- **The handoff's "first-seen" list is inaccurate against master's seed and was NOT used as-is.** Reconciled all 70 raw names against the 70-row seed floor: `cystatin_c` is already seeded (the handoff wrongly lists it first-seen); the handoff OMITS three genuinely-unmapped markers (`CK`, `Bilirubin conjugated`, `Free PSA`); and every proposed key that collides with a seeded analyte is wrong (proposed `hdl`/`ldl`/`total_testosterone`/`free_testosterone`/`psa_total`/`red_cell_count`/`white_cell_count`/`zinc`/`vitamin_d`/`non_hdl_c` vs seed `hdl_cholesterol`/`ldl_cholesterol`/`testosterone_total`/`testosterone_free_calculated`/`psa`/`rbc`/`wcc`/`zinc_plasma`/`vitamin_d_25oh`/`non_hdl`). `confirm` stores the seeded ones correctly (map wins), which is exactly why auto-accepting proposals as *declare* keys would fork series (#50). The **live `/labs/canonical-map` is authoritative** (the seed is only the migration floor, #220); the confirmation screen's `unmapped[]` is the real bind trigger.
- **Over-collapse landmines are non-issues on master.** total-T (`testosterone_total`, nmol/L) and free-T (`testosterone_free_calculated`, pmol/L) are **distinct seeded canonicals** — nothing to merge, the unit guard is only a backstop; standalone (0902) and Androgens-panel (0804) `Testosterone` share the raw name → collapse to one series, correctly.
- **Three canonical binds RATIFIED (operator, this session)**, to execute operator/prod-side via `POST /labs/canonical/bind`: `IGF-1 (Liaison)`→`igf1` (nmol/L — "(Liaison)" is an assay tag; a future non-Liaison IGF-1 raw name binds to the same `igf1` as its own bind row); `Free PSA`→`psa_free` (ug/L — pairs with seeded `psa`); `% Free PSA`→`psa_free_percent` (%). The remaining unmapped markers (`ACTH`, `Cortisol am`, `Free T4`, `CK`, `Bilirubin conjugated`, and the 8 urine M/C/S markers) were NOT decided — they store unmapped (retain-raw #58/#155) and bind later.
- **Imaging / DEXA target DEFERRED (operator).** MRI/US narrative + structured DEXA numerics have no landed home; forcing them into `lab_results` is refused. Tracked as Q142.

**Rationale.** Mints no data-meaning of its own — table/flag/unit meaning is relayed from master as already recorded, and the two operator calls (binds, defer) are ratified, not invented. The batch is NOT stored by this session because the store is a prod write through the confirmation screen (operator/prod-credentialed, irreversible under retain-raw #155) that embeds marker-meaning binds — Merge disposition hold (c). The extraction JSON is NOT committed to the repo (kill-rule: lab data lives in the prod DB and project knowledge, never in git).

**Status.** ADJUDICATED + RATIFIED (governance-only: this entry + Q142 + BRANCHES + closeout; no code, no migration, no prod write). Self-merged on green — implements exactly the operator's two ratified calls with no new judgment. **OWED (operator, prod-side):** run the three `/labs/canonical/bind` calls, then the 24 confirms at `/labs` against prod; verify via Postgres against Railway (not on-device UI) — an unseeable surface this session cannot confirm.

**How you know.** Seed diff computed this session (`backend/reference/marker_canonical.json`, 70 rows) against the batch's 70 distinct raw names: 54 already seeded (auto-bind at confirm), 16 unmapped-vs-seed. `backend/routers/labs.py::confirm_lab_report` resolves by `marker_name_raw` and stores unmapped rows `marker_canonical=NULL` (retain-raw #58/#155); `bind_canonical_marker` is the operator-supplied exact-bind path carrying the §6 over-collapse guard at bind- and backfill-time. The two 09-02 events carry disjoint marker sets (verified over the parsed payload).

**Do not revisit unless.** The `health_events` narrative parent (#43/#52) lands or a dedicated numeric-imaging series is ratified (Q142 becomes buildable); a batch-confirm endpoint is added (today it is one report per `/labs/confirm` call); or a canonical bind is later found to have forked a series (then the live-map-authoritative rule here is the thing to check first).

### 281. `schedule_item` day-overlap refines to day AND time overlap (supersedes #233's day-alone trigger); retirement lever confirmed as the `active` column, not `season_end`; MCP tool outputs carry an `as_of` anchor

**Context.** A decompression-schedule session's writes bounced en masse: every weekday schedule write refused with `lands on a day already held by id 5 — Work (morning)` despite Work=morning and gym=16:30; all three rugby-retirement writes refused with `unknown field(s) ['active']`; only Sat/Sun persisted. Verify-then-fix brief, four workstreams. Each hypothesis was adjudicated against master before any edit.

**Decision (WS1 — the schema decision).** The `schedule_item` day-overlap check (`routers/knowledge.py::_schedule_overlap_check`) now triggers on **day overlap AND clock-time overlap**, refining #233's **day-membership-alone** trigger. Two same-day commitments at non-overlapping times (work-morning, gym-16:30) save with no `supersedes`/`distinct_from`; the acknowledgement prompt is reserved for a genuine same-time collision. Time is resolved to a `[start, end)` interval per row (`_time_interval`): a parseable `time_range` wins ("HH:MM-HH:MM", or a lone "HH:MM" as the point `[t, t]`), else the `time_of_day` band (morning 05:00–12:00, afternoon 12:00–17:00, evening 17:00–22:00). **Unresolvable time (`time_of_day` "unknown"/absent with no parseable `time_range`, or a present-but-unparseable `time_range`) is treated as possibly-overlapping** — the ambiguous case stays visible, never a silent double-book. The anti-duplicate guarantee #233 was written for survives: a true duplicate (a new key minted for an existing commitment) shares that commitment's time, so it still collides. Matching on `activity` stays rejected for #233's reason (string equality over LLM free text fails OPEN). The bands are a data-meaning default introduced here; the brief ratified "pass when times don't overlap" and delegated the specifics with instruction to log — so this self-merges as chat-approved implementation, not a new un-ratified fork.

**Decision (WS1#4 — field-message reconciliation).** The overlap refusal advises `distinct_from`, but the `unknown field(s)` refusal printed only the stored `SCHEDULE_ITEM_FIELDS` list — a writer that hit the shape refusal was told a field the same surface elsewhere tells it to use is unknown. The refusal now also advertises the write-only acknowledgement fields (`distinct_from`). `distinct_from` was **always an accepted write field** (`SCHEDULE_ITEM_WRITE_ONLY_FIELDS`, honoured in `validate_schedule_item`'s `known` set) — the brief's hypothesis that following the clash advice earns a second rejection is **REFUTED**; only the advertised list was incomplete.

**Decision (WS2 — retirement, COUPLED to WS1).** The brief's mechanism hypothesis is **REFUTED and the actual mechanism recorded**: `active` is a **real stored column** on `user_knowledge_entries` (not a computed/read-side attribute) — it renders T/F on read via `KnowledgeEntryOut.active` because it is a genuine column, and is rejected as `unknown field(s) ['active']` only when placed **inside `value`** (where it is not a field). The retirement lever is that **top-level `active: false`** deactivation (chat block path `routers/chat.py`, and the `resolve` endpoint / expire-stale sweep — the three writers of #269 all flip this same column), **NOT `season_end`**: `season_end` is inert by design (`context_builder`: "Nothing retires itself: a passed date is a prompt, never an automatic change of state"). The WS2-critical requirement — **does the clash query exclude retired rows?** — is **already satisfied**: `_schedule_overlap_check` filters `active=True`, so a deactivated row no longer occupies its day. So the coupling gate passes with WS1 fixed + correct retirement usage; no change to the overlap-vs-retirement interaction was needed. The real WS2 defect was discoverability: the agent mis-nested `active` into `value`. Two in-tree fixes — the `unknown field` refusal now redirects `active` to the top-level lever, and `context_builder`'s STEP-2 span teaches the top-level deactivation block explicitly (kept inside the #43-parity-excised STEP1→STEP3 span so `test_current_state` parity holds; CONTRADICTION HANDLING left byte-identical).

**Decision (WS3 — temporal anchoring).** Every MCP tool output (`mcp_server.py`, all 7 tools) now carries a visible `as_of: <ISO> (Australia/Brisbane)` line via a `@_stamped` decorator over `_stamp`/`_as_of` (Brisbane, no DST — the tz `context_builder`/`load_metrics` already anchor on). The data was already emit-time fresh (every query resolves `CURRENT_DATE` server-side per call); what was missing was a *visible* anchor, so a thread resumed on a later day could reason off a stale cached "today" (the Q140 skew family). **Flagged as out-of-repo (Q143):** the per-turn datetime re-anchor into the MCP client's context and the forced readiness/schedule re-pull on a day-boundary crossing live at the chat/client/orchestration layer, not in health-app.

**Rationale.** WS1 moves the failure from "refuses every distinct same-day commitment" to "refuses only a real time collision or an unprovable one" while keeping #233's silent-duplicate guarantee. WS2's mechanism was refuted, so per the brief no fix was reshaped to force it — the confirmed lever already works; only discoverability was repaired. WS3 makes the anchor legible without changing the (already-fresh) data.

**Status.** LANDED (code + tests + governance, one branch; no schema migration, no prod write). Self-merged on green — chat-approved brief, schema specifics logged here as directed. **WS4 finding (no code):** all MCP tools return bare `str`, so a failure is a string the agent can gloss (the observed "saved/locked" narration over `✗` returns); the chat-side schedule writes DO already return structured `✓`/`✗`-prefixed action strings, and the acknowledgement discipline (an orchestration/agent-prompt concern) is flagged in Q143, not forced into the tree.

**How you know.** `_schedule_overlap_check` reads `_times_overlap(value, row_value)` gated behind `days & row_days`; `backend/tests/test_schedule_item_schema.py` pins: non-overlapping bands coexist, same band still collides, lone-time-in-band collides, disjoint time_range coexists, unknown/unparseable time still collides, `active`-in-value redirects to top-level, the unknown-field message names `distinct_from`, and the coupling gate (retire rugby via `active` → gym-16:30 saves beside work-morning on the same Tuesday). `backend/tests/test_mcp_as_of_stamp.py` pins the Brisbane stamp, later-date-sorts-later, the UTC-evening→next-Brisbane-day skew, and that `functools.wraps` preserves the name/docstring FastMCP introspects. Full suite 1465 passed (the 5 Garmin-lane tests need Python ≥3.12, unavailable in the 3.11 verify sandbox; none touch these files). `test_current_state` #43-parity green (prompt edits confined to the excised span).

**Do not revisit unless.** A same-time collision is wanted to auto-pass too (it must not — that is the silent-duplicate hole #233/#281 exist to close); the time bands need finer grain than morning/afternoon/evening (then `_TIME_OF_DAY_BANDS` is the seam, and a `time_range` already overrides them); `season_end` is ever made an automatic retirement lever (a deliberate reversal of the #233-era "nothing retires itself" rule — decide it in chat first); or the MCP client gains its own per-turn `as_of` injection (then the tool-side stamp becomes belt-and-suspenders, not the anchor — Q143).

### 282. Lab batch ingestion (#280) DISCHARGED & verified in prod

**Decision.** The operator-owed, prod-side ingestion left OWED by #280 is **complete and independently verified this session**. The SNP batch is in prod: **24 report groups across 2026-08-04 (13) and 2026-09-02 (11)**; the three canonical binds ratified in #280 are live — `igf1` (`IGF-1 (Liaison)` 39.0 nmol/L, 09-02), `psa_free` (`Free PSA` 0.12 ug/L, 09-02), `psa_free_percent` (`% Free PSA` 21.8 %, 09-02); marker series are continuous, with the late-bind backfill visible where the new draws join prior history (`testosterone_total`, `fsh`, `prolactin`, `psa`); `Zinc-plasma` 2026-08-04 (12.9 umol/L) is present. No prod-side OWED remains on #280. Imaging/DEXA stays deferred (Q142).

**Rationale.** #280 recorded the adjudication and explicitly held the prod write as operator-owed ("an unseeable surface this session cannot confirm"). This entry closes that loop now that the surface IS readable this session (the `Health app data` MCP `get_lab_results` tool is connected) — so the discharge is recorded against an independent read, not a relayed status. Append-only: #280's locked text stands; this supersedes only its OWED status.

**Status.** DONE. Governance-only (this entry + `closeout.md` strike + CLAUDE Recent-landings + BRANCHES row; no code, no migration, no prod write by this session). Self-merged on green.

**How you know.** `get_lab_results` (report-grouped, `as_of 2026-09-12T14:56:44+10:00`) read this session: counted 24 batch report groups over exactly 08-04 and 09-02 (no 09-03 exists — the handoff-note date list saying "03-Sep" is inaccurate against prod and was NOT used); the three bind markers render on their 09-02 reports; Testosterone (6 draws incl. 09-02/08-04), FSH, Prolactin and PSA render as continuous series joining the batch to prior draws; `Zinc-plasma` 08-04 present; no within-date duplicate report groups in the read-layer grouping. Reconciles the operator's own 2026-09-11/12 `get_lab_results` verification.

**Do not revisit unless.** A `lab_reports`/`lab_results` row-level audit (Postgres against Railway) later finds a duplicate or a series fork the read-grouped view could not show (then #280's live-map-authoritative bind rule is the first thing to check); or the imaging/DEXA deferral (Q142) is taken up.

### 283. Structured save-result contract on the chat knowledge/schedule write lane (Q143a / WS4)

**Context.** The origin transcript (decompression-schedule session) narrated "saved / locked" while the underlying writes returned `✗`. The `✗` was present in `actions_taken` and still glossed, because success/failure was only inferable from prose the model is trusted to read. WS4 asked for a machine-checkable outcome so a failed write cannot be narrated as success. Verify-then-fix; the brief's premise (that the write tools are the `mcp_server.py` MCP wrappers) was checked first.

**Verification (against master).**
- **The write surface is the CHAT channel, NOT an MCP tool — premise REFUTED.** `mcp_server.py` has zero write tools; all seven `@mcp.tool()`s are `get_*` reads. The agent-narrated schedule/knowledge write path is `routers/chat.py::_process_knowledge_updates`, which parses `<knowledge_update>` blocks out of the assistant turn and returned `ChatResponse{response, actions_taken: list[str]}` — bare `✓`/`✗`/`⚠️`/`ℹ️` strings. So "ride the structured field alongside `as_of` at the `@_stamped` boundary" does not apply: `@_stamped` wraps read-tool string outputs, and there is no write tool there.
- **The HTTP surface is ALREADY structured — descoped.** `routers/knowledge.py::create_entry` (`POST /knowledge/entry`) returns `201`+`KnowledgeEntryOut` on success, `409` `{error, message, overlapping, resolve_with}` on overlap, `422` on shape. Machine-checkable via status code + typed body; no change. `routers/health.py` session-analysis is a `source="system"` internal write, not an agent-narration surface.
- **The consuming gate has no in-repo (health-app) target.** `frontend/` has no chat component and no `/chat` caller; the endpoint is consumed by the companion app / an external agent client. The layer that would refuse to emit "saved" on `saved:false` lives there, not here. Recorded to Q143.

**Decision.** `ChatResponse` gains `write_results: list[WriteResult]`, additive to `actions_taken` (preserved verbatim). `WriteResult = {saved: bool, reason_code: str, reason: str, key: str|None}`, **one per `<knowledge_update>` block — never a batch roll-up** (the origin failure was N blocks where some saved and some did not, narrated as all-saved; a single top-level flag would reintroduce exactly that gloss). A local `record()` appends to `actions_taken` and `write_results` together, so `saved` and the prose can never disagree. **`reason_code` vocabulary (stable, TYPE-derived, never message-parsed):** `saved` (create/update success) · `deactivated` (top-level `active:false` retired a row) · `not_found` (deactivation named a key with no active row — a non-write, not a success) · `day_time_clash` (`ScheduleItemOverlap`, carries the code as a class attribute) · `unknown_field` (`ScheduleItemInvalid` with `code="unknown_field"`, raised at the unknown-key site — the WS2 failure, now typed) · `invalid_shape` (any other `validate_schedule_item` `ValueError`) · `invalid_json` (block did not parse) · `error` (unexpected). `ScheduleItemInvalid` subclasses `ValueError`, so every existing `except ValueError` (HTTP→422, chat) keeps catching it unchanged.

**Rationale.** The fix is substrate only — it makes hard-gating POSSIBLE; enforcement is the out-of-repo consuming gate (Q143). Type-derived codes over message-parsing per #103 (controls discriminate on identity, not prose). Scoped to the knowledge/schedule lane the origin failure occurred on; the Hevy routine/exercise action strings are left string-only, with `WriteResult` as the shape a later session reuses for them.

**Status.** LANDED (code + tests; non-migration, self-merged on green). Chat-approved brief; the return shape and code vocabulary are logged here as directed.

**How you know.** `backend/tests/test_chat_knowledge_channel.py` pins: a save → `saved:true`/`saved`; a day+time clash → `saved:false`/`day_time_clash`; `active` inside `value` → `saved:false`/`unknown_field`; deactivating a missing key → `saved:false`/`not_found`; a real deactivation → `saved:true`/`deactivated`; a mixed three-block batch → per-row `[True, False, True]` (not a roll-up); and `len(actions)==len(results)` with `reason`==the action string (the pairing invariant). Full suite 1468 passed (the 5 Garmin-lane tests need Python ≥3.12, unavailable in the 3.11 verify sandbox; none touch chat/knowledge).

**Do not revisit unless.** The consuming gate is built (then confirm it reads `write_results[*].saved`, not the prose — Q143); the Hevy routine/exercise lanes need the same guarantee (extend `write_results` with their own codes, e.g. `hevy_not_connected`/`hevy_rejected`, rather than forking a second envelope); or a finer shape-error taxonomy than `unknown_field`/`invalid_shape` is wanted (then give `ScheduleItemInvalid` more codes at the raise sites — never parse the message).

---

### 284. Narrate-after-write on the chat knowledge/schedule lane — regenerate on a failed write, with a deterministic floor (Q143a / WS4)

**Context.** #283 made each `<knowledge_update>` write's outcome machine-checkable (`WriteResult.saved`), but the reply the user reads is still generated in ONE pass BEFORE any write executes — `routers/chat.py` sets `reply = response.content[0].text` and only then calls `_process_knowledge_updates`. So a "saved" the model writes into that prose PRECEDES the outcome it claims; the origin gloss (rugby-retire narrated as done while the mis-nested `active` block was refused) stayed reachable in the prose even with #283's structured field present. #283 built the substrate; this closes the loop on the NARRATION. Verify-then-fix: the single-pass ordering and the #283 return shape were re-asserted against master before building.

**Decision.** After the writes run, `_compose_response` branches on `write_results`:
- **every write saved (or none attempted)** → keep the pre-write `reply` unchanged, and fire NO second model call;
- **any write failed** (`saved:false` — `not_found` included) → DISCARD the pre-write prose and run a bounded second generation (`_narrate_after_write`, `max_tokens=1024`, the SAME Anthropic client) that reproduces the turn's conversational/analytical content and reports the true per-row outcome.
Affordance is TYPE-derived from `reason_code` (never prose-parsed, per #103): `day_time_clash`/`needs_disambiguation` → `user_resolvable`, offer the supersedes/distinct_from path; `unknown_field`/`invalid_shape`/`invalid_json`/`error` → `system_issue`, our fault — flag it and NEVER tell the user to fix a field/format/block; anything else (e.g. `not_found`) → `informational`. An **always-on** deterministic footer rendered from `write_results` (`✓ N saved` / `⚠ X saved, Y failed — <distinct reasons>`) is appended to `response` under BOTH paths — the hard floor if pass-2 itself misreports. Defence-in-depth in `context_builder.py` STEP 2 (inside the parity-excised STEP 1→STEP 3 span): the model REQUESTS a write and the system reports the outcome, so state intent, never completion.

**Rationale.** The guarantee is the floor, not the model — pass-2 is best-effort truthful narration; the footer is deterministic and computed from `write_results`, not the prose. Always-on over failure-only (the [fork], ratified always-on by the operator this session): one terse `✓ N saved` line on success turns is the accepted cost of also neutering the exotic "narrated a save it never emitted" case (the reader gets a true tally to cross-check an over-claiming sentence); failure-only left that case uncaught (residual recorded to Q143). Cost profile: **+1 bounded model call on failed-write turns ONLY** — an all-saved turn is unchanged bar the footer line, a non-write turn is untouched. The footer is server-side, so it reaches the web `ChatPanel` (which renders `data.response` verbatim, `ChatPanel.jsx:78`) and any other `/chat` client with no per-client change.

**Status.** LANDED (code + tests; non-migration, self-merged on green). Chat-approved brief; the footer-scope [fork] was ratified always-on by the operator this session — no other un-ratified call embedded.

**How you know.** `backend/tests/test_chat_narrate_after_write.py` (transport-faked per #166) pins: an all-saved turn fires NO second call and only appends `✓ N saved`; a `day_time_clash` fires exactly one call, leaves no surviving false "saved", offers the resolve path, footer `⚠ 0 saved, 1 failed — day/time clash`; a mixed 2-saved/3-failed batch reports per-row (five keys named individually, never a roll-up) and hands the draft to pass-2 to preserve; a bug code is tagged `system_issue` and the pass-2 system prompt forbids handing it back to the user; the origin rugby-retire-plus-gym/swim replay produces a truthful `response`; a pass-2 transport error falls back to a fixed safe line + floor, never the false draft. Parity guard `test_current_state` green — the `context_builder` edit is confined to the excised span. Full backend suite **1502 passed** (the Garmin lane needs Python ≥3.12, unavailable in the 3.11 verify sandbox; none touch chat).

**Do not revisit unless.** The Hevy routine/exercise lanes need the same guarantee (map them onto `write_results` with their own codes per #283, then they flow through `_compose_response` unchanged); or a client wants the footer suppressed on success (revisit the always-on fork — but the un-emitted-save case then returns, Q143); or a finer affordance taxonomy than `user_resolvable`/`system_issue`/`informational` is wanted (extend `_write_affordance`, keep it type-derived).

### 285. Hevy routine read-back — an MCP read surface (list + full-by-id), mirroring the workout read-back (Q144)

**Context.** The model could create routines (chat `<hevy_create_routine>`) but could not SEE them: workout read-back existed as an MCP tool (`get_hevy_workouts`), routine read-back existed only as a raw `.json()` REST passthrough (`GET /hevy/routines`, `integrations.py:332-343`) with no model-facing formatted layer. So a routine could not be confirmed made (exists) or made correctly (config matches intent). This is the foundation a future create-guard (Q144 finding a) consumes and the surface that verifies superset writes later. Verify-then-build: all six VERIFY items re-asserted against master before building — the connector `get_routine(id)` method was confirmed ABSENT (only `get_routines` list existed), routines confirmed not persisted (no model, nothing to reconcile).

**Decision.** Add `HevyClient.get_routine(id)` (`GET /v1/routines/{id}`) and two MCP read tools, live-read, text output, no persistence — mirroring `get_hevy_workouts` (decrypt stored key, live call, paginate `page < page_count` at `page_size=10`, parse, format):
- **`search_hevy_routines`** — compact list: per routine title, id, folder, exercise titles + count, NO per-set detail. The cheap title-existence check a create's check-exists-before-create guard wants.
- **`get_hevy_routine(routine_id)`** — full config of one routine incl. superset grouping (`[superset N]` tag, in payload order — the `is not None` test admits a falsy `superset_id` 0), rest, notes, and every set via the shared `hevy_format.format_set`. The "made-correctly" verification surface.
Exercise names render as titles: payload `title` first, `HevyExerciseTemplate` store fallback (`catalogue_titles_by_id`) ONLY for exercises whose live payload omits one — zero store queries on the happy path (VERIFY 4). Empty (no routines) and bad-id (404 → clean not-found) handled without a crash; the by-id GET tolerates both a `{"routine": {...}}` wrapper and a bare object (the connector parses nothing, so neither shape is asserted).

**Granularity [fork] — SPLIT (list + full-by-id), operator-leaned.** Over one fuller list tool. Prior-art-validated (`chrisdoc/hevy-mcp` splits `search-routines`/`get-routine` to keep context small — reference for shape, not a dependency): the compact list keeps a title/existence check context-small and is exactly what the create-guard's idempotency check reads; the full view is what correctness verification wants; it scales past a handful of routines. Read-live/no-persistence and text-output (not raw JSON) were operator-stamped.

**Status.** Code + tests complete; opened as a PR to master under the PR-gated merge path (#171), merges on green per § Merge disposition (non-migration feature). Implements a chat-approved brief with no new judgment beyond the operator-leaned granularity fork — already ratified.

**How you know.** `backend/tests/test_mcp_hevy_routines.py` (12 tests, Hevy client faked per the `get_hevy_workouts` test pattern — stored key a real Fernet token so `decrypt` runs unmocked) pins: compact list renders title/id/folder/count and omits set detail; pagination concatenates page 1..page_count; full-by-id returns superset grouping (incl. the falsy `superset_id` 0), rest, multi-line notes, warmup label + RPE, per-set detail; titles render not UUIDs, with the store fallback exercised when the payload omits a title; empty, bad-id (404), `{"routine":...}` wrapper, and not-connected all return clean messages; output carries the `as_of` stamp. Full backend suite **1493 passed, 0 failed** minus the Py3.12 Garmin lane (5 files import `garminconnect`, which requires Python ≥3.12, unavailable in the 3.11 verify sandbox — none touch Hevy); the one non-Garmin miss in a first pass was environmental (`test_current_state` `git show`s a historical blob, needs a full clone — green on `--unshallow`).

**Do not revisit unless.** Superset WRITE support is added (surface `superset_id` in the create prompt/contract — the connector already plumbs it, only the `_section_routine_creation` block omits it; this read-back is its verification surface); or the Hevy action lanes are mapped onto `write_results` (#283/#284/Q143 — read-back verification could fold in there); or routines need persistence (currently live-only, matching workout read-back's live path); or the granularity split proves wrong at scale (collapse to one tool).

### 286. Hevy routine-create hardening — rpe strip, title+folder idempotency guard, WriteResult fold (Q144 a–c)

**Context.** The model could create routines and (post-#285) see them, but the create path was unhardened: it emitted `rpe` on routine sets (Hevy ignores it — a workout-set field carried over), it POSTed blindly with no existence check against an API that has NO delete (a retry or an unchecked create mints an undeletable duplicate — Q144(a)), and its outcome was string-only (`_process_routine_actions` returned `(cleaned, actions)`), so a failed or collided create could be narrated as success — the exact gloss #283/#284 closed on the knowledge lane. Verify-then-build against master (post-#285, lines shifted from the brief's survey): superset plumbing confirmed intact end-to-end (`connectors/hevy.py` `ex.get("superset_id")`, `integrations.py:80` model field, chat pass-through through `_resolve_missing_ids` which never touches it); confirmed there is NO workout-CREATE path at all (Hevy workouts are read-only here; `rpe` on the workout side lives only on the READ/parse path in `load_events.py`), so stripping rpe from routines cannot touch load logging — cleaner than the brief's "separate method" assumption.

**Decision.** Harden in the connector floor so both entry points (chat, REST) inherit it:
- **rpe strip (b).** Remove `rpe` from the routine set-field tuple in `HevyClient.create_routine`. Deterministic, not prompt-only — the model may emit it anyway. The workout read path is untouched (there is no workout-create path).
- **Idempotency guard (a) — refuse + offer, title+folder [fork, Luke].** `create_routine` reads the paginated `/routines` list (new `get_all_routines()`, mirroring `get_all_workouts`) and, before the POST, raises typed `RoutineAlreadyExists` (carries the colliding rows, `code="already_exists"`, mirroring `ScheduleItemOverlap`) on a collision. Match is **case-insensitive on title AND equal on folder_id (None==None)** — same title in a different folder is a distinct routine. Refuse + offer over auto-update or refuse-silently: Hevy has no delete and update replaces contents, so auto-updating a false title match is unrecoverable. Cost accepted: one `/routines` read per create.
- **WriteResult fold (c).** `_process_routine_actions` returns `(cleaned, actions, write_results)` — one `WriteResult` per block, paired 1:1 with the action string via a `record()` helper (the #283 pattern). Hevy reason_code vocab (locked to house style): `created` | `already_exists` | `unresolved_exercise` | `invalid_json` | `invalid_shape` | `create_failed`. Assembly combines routine + knowledge `write_results` into the single narrate-after-write pass (#284) and the deterministic footer. `already_exists` is `user_resolvable` (rename vs update, exactly like `day_time_clash`); `invalid_json`/`invalid_shape` are `system_issue`; the rest are informational. REST lane catches `RoutineAlreadyExists` → HTTP 409, mirroring `create_entry`.

**Status.** Code + tests complete; lands with #287 on `claude/lucid-galileo-h5340t` under the PR-gated merge path (#171), non-migration, self-merges on green per § Merge disposition. The two [fork] design calls (collision behaviour, title-match strictness) were put to Luke and ratified this session — collision = refuse+offer, match = title+folder — so the embedded judgment is chat-approved, not new at merge.

**How you know.** `backend/tests/test_hevy_routine_hardening.py` fakes the TRANSPORT and drives the real `HevyClient` (the layer where the strip and guard live, invisible to a FakeHevyClient): rpe stripped from every routine set (weight_kg/reps survive); a same-(title,folder) create raises `RoutineAlreadyExists` and never POSTs; case-insensitivity; folder discriminates (same title, different folder → created; None vs set → created); superset_id reaches the payload grouped and a falsy `0` survives as a real group; the #285 full-render reads a grouped `superset_id: 0` back as `[superset 0]` (round-trip read half); REST collision → 409. `backend/tests/test_chat_routine_write_results.py` pins the fold: collision → `saved=False, code=already_exists`, paired with its action string; `already_exists` is `user_resolvable`; generic failure → `create_failed`; not-connected is a failed write; a collided routine write reaches the deterministic footer. `test_routine_provisioning.py` extended for the created + unresolved WriteResults. Full backend suite **1521 passed** minus the Py3.12 Garmin lane (5 files import `garminconnect`, needs ≥3.12; `test_hc_sync_diagnostics` transitively among them — fails identically on clean master, environmental). Parity guard green on `--unshallow`.

**Do not revisit unless.** The `@`-in-notes convention needs a hard connector strip (documented in the prompt only this pass — stripping legitimate note text was NOT done without a live Hevy confirmation that `@` breaks a routine create, per the verify-before-strip discipline #103/#166); or the Hevy **exercise-action** lane (`_process_exercise_actions`) is folded onto `write_results` — the last string-only Hevy write surface (Q144); or Hevy adds a delete/PUT that makes auto-update on collision recoverable (revisit refuse+offer); or the collision read cost (one `/routines` walk per create) proves too dear at scale.

### 287. Superset write support — `superset_id` prompt contract + routine-create conventions (Q144 d)

**Context.** #285 confirmed the connector and REST model plumb `superset_id` end-to-end and read it back grouped (`[superset N]`), but `context_builder._section_routine_creation` never documented the field, so the model could not EXPRESS a superset — the plumbing sat dormant. Unilateral pairing (alternating the two sides of a single-arm/leg movement) is Easty's stated priority. The same section also omitted several fragile conventions the create path silently depends on.

**Decision.** Extend `_section_routine_creation` (built on #286's hardened base):
- **`superset_id` documented** — shared integer = one superset (performed together / alternated), null or omitted = standalone, different integers = different groups.
- **Unilateral pairing is the highlighted worked example** (Easty's priority): both sides in one group share a `superset_id`.
- **`superset_id: 0` called out** as a real group distinct from null — the falsy-zero trap the connector already avoids (`ex.get`, not `x or None`).
- **Conventions baked in:** ≥1 set per exercise; snake_case field names; omit `weight_kg` (never send `0`) for bodyweight — 0 is a real load; `rest_seconds` defaults to 90; no `rpe` on routine sets (Hevy ignores it; #286 drops it regardless); no `@` in notes; a belt-and-suspenders nudge to `search_hevy_routines` before creating, with the connector guard (#286) named as the hard floor.
- **"What to superset" is out of scope** [stamp]: #287 makes superset expression possible and correct; which movements to pair is training-chat's judgment, not encoded here.

**Status.** Code + tests complete; lands with #286 (concern-split feature commits, one gov commit). Non-migration, self-merges on green.

**How you know.** Parity-safe by construction: `_section_routine_creation` is already EXCISED from `test_current_state`'s pre/post-refactor guard (`connected_integrations=[]` → the section returns `""` on both sides, #80/#82), so this edit cannot drift that guard — determined before writing the prose, not after. `test_current_state.py` green on `--unshallow`. The section is pinned instead by `test_routine_creation_prompt.py`, extended here: `superset_id` documented, "unilateral" is the example, `"superset_id": 0` distinguished from null, and the conventions (snake_case, ≥1 set, omit-weight-not-zero, no rpe, no `@`). All 35 prompt/catalogue-visibility tests green.

**Do not revisit unless.** Hevy changes the superset grouping semantics; or the create prompt needs the `@`-strip promoted to a hard connector floor (see #286 "Do not revisit"); or a future arc encodes movement-pairing judgment (currently training-chat's, deliberately not here).

### 288. Hevy exercise-action lane folded onto WriteResult — the last un-wrapped write surface (Q144)

**Context.** #286 folded the routine lane onto the #283 `WriteResult` substrate and flagged the custom-exercise lane (`_process_exercise_actions`) as the last string-only Hevy write surface — a failed/rejected/unconfirmed create could still be narrated as success there, the exact gloss the acknowledgement-discipline arc (schedule #281/#283/#284, Hevy routine #286) exists to close. Verify-then-build against master (post-#287): `_process_exercise_actions` confirmed a 2-tuple; callers enumerated (chat endpoint + test sites in three files; no `probe_resolver` caller, unlike the routine lane); the create path is `create_and_resolve` (#65) → connector `create_exercise_template` (typed 403 `HevyCustomExerciseLimitError` / 400 `HevyBadRequestError`, tolerant of a bad 2xx body), raising `HevyKeyMissingError` / `HevyCreateUnresolvedError`.

**Decision — wrap-only fold, no collision guard.** `_process_exercise_actions` returns `(cleaned, actions, write_results)`, one `WriteResult` per block via a `record()` helper paired 1:1 with the action string. Assembly feeds exercise + routine + knowledge `write_results` into the single narrate-after-write pass (#284) and the deterministic footer.

**Idempotency [fork, conditional on VERIFY 7] — resolved to NO guard.** VERIFY 7 found the lane is already idempotent at three layers: the honest-confirmation `resolve_exercise` pre-check in `_process_exercise_actions`, `create_and_resolve`'s own #65 pre-check ("only mint when genuinely absent") before the non-idempotent POST, and the #212 freshness gate that closes the stale-catalogue mint window. Duplicate creation is structurally prevented, so — unlike the routine lane, where nothing checked before the POST — no `RoutineAlreadyExists`-style guard is added. The fork's stated lean ("don't add it if the risk doesn't exist") governs; this is the verified condition, not new judgment.

**Reason_code vocab [stamp].** Reuses #286 where the outcome matches (`created` saved=True; `invalid_json`/`invalid_shape` saved=False, system_issue; `create_failed` saved=False). New codes for the genuinely distinct exercise outcomes, all `informational`:
- `already_present` (saved=False) — the title is already in the catalogue; nothing minted. saved=False deliberately: nothing was written, so a would-be "created!" is corrected to "already in your catalogue" (ℹ️, not ✓) rather than left standing — distinct from the routine `already_exists`, which is a `user_resolvable` refusal (rename vs update). A benign idempotent no-op here, not a choice.
- `limit_reached` (saved=False) — Hevy custom-exercise account limit (403).
- `invalid_exercise_field` (saved=False) — Hevy 400 rejected the body/enum; the message names the field + valid enum for correction.
- `created_unconfirmed` (saved=False) — POST likely succeeded but the list-back never surfaced it; the verbatim "do NOT create it again" guidance rides the action string regardless of how pass-2 renders it (a second create against a delete-less API is how you mint a permanent duplicate).

**Status.** Code + tests complete; non-migration, self-merges on green per § Merge disposition. Implements a chat-approved brief with the conditional fork resolved by the verified idempotency finding — no new judgment at merge.

**How you know.** `backend/tests/test_chat_exercise_write_results.py` (11 tests, chat helpers monkeypatched to isolate the orchestration) pins every outcome: `created` saved; `already_present` saved=False + ℹ️ (not narrated as created); `limit_reached` / `invalid_exercise_field` / `created_unconfirmed` (with the verbatim no-retry guidance) / `create_failed`; `invalid_json` / `invalid_shape`; the four new codes are `informational` (never `system_issue`/`user_resolvable`); exercise writes feed the footer. Every caller updated to the 3-tuple (grep-clean, GATE). The connector-level create behaviour stays covered by `test_hevy_create_response_tolerance.py` (transport-faked). Full backend suite **1528 passed** minus the Py3.12 Garmin lane (5 files import `garminconnect`; `test_hc_sync_diagnostics` transitively among them — fails identically on clean master, environmental).

**Do not revisit unless.** Hevy adds a delete for custom exercises (revisit whether `already_present` should offer a replace path); or a consumer needs the exercise `write_results` to branch on a distinction these codes don't carry (add a code, logged); or the exercise-create idempotency layers change (the no-guard decision rests on all three holding).

### 289. Routine-create numeric floor + folder name→id resolution — root cause of `Expected number, received nan` (Concern A)

**Context.** Three decompression routines all failed create with Hevy's `Expected number, received nan` 400. Verify-then-fix (the chat's suspects — folder coercion, distance_meters/duration_seconds — were NOT patched on faith): reading the code, the chat lane reads `data.get("folder_id")` and drops the model's `folder` NAME entirely; there is no `float()`/`int()` coercion anywhere in the routine path. So folder was never the NaN source (it's a separate silent-drop gap). Reproduced the real mechanism offline: `json.loads` accepts a literal `NaN` token (`parse_constant` default) → the builder's `if val is not None` passed it through (`NaN is not None`) → httpx serialized it (`allow_nan=True` default) straight to the wire. The failing field is whatever numeric slot the model emitted `NaN` in — the fix closes the CLASS, not one field.

**Decision — deterministic numeric floor (both floors), reject-and-name.** [fork ratified by Luke: both floors, surfaced as a named WriteResult.]
- **Named pre-POST reject (early catch):** the connector checks every value bound for a numeric field (weight_kg, reps, distance_meters, duration_seconds, rest_seconds, superset_id, folder_id) for finiteness BEFORE any network call and raises typed `RoutineNumericError` (`code="invalid_number"`, a ValueError subclass so REST's `except ValueError` still catches) naming the field + set + exercise. NaN, ±Infinity, and a string/bool in a numeric slot are all rejected; `0` stays valid (a real load — distinct from the omit-weight-not-zero prompt guidance). A non-finite is never built.
- **Hard serialize backstop (never-sent guarantee):** the POST uses `content=json.dumps(payload, allow_nan=False)`, so anything that slipped the floor raises locally rather than emitting an invalid `NaN`/`Infinity` token.
- **Deliberately NOT `parse_constant`:** Luke's "parse floor" was implemented as the post-build named reject + allow_nan=False, not a `json.loads(parse_constant=…)` that aborts — because parse_constant rejects the token before the field is known, and naming the field+exercise was the stronger requirement (a bare "invalid number" on a 7-exercise routine is a needle-in-haystack). Same ingress-catching effect (caught before the create), plus it names the field. Flagged to Luke; a strict token-reject can be added as defense-in-depth if wanted.
- Chat lane folds `RoutineNumericError` → `invalid_number` WriteResult (system_issue — a malformed payload is our-side, never handed to the user to fix); REST lane → 422.

**Folder name→id resolution (FIX #1).** The `folder` name was silently dropped, landing every routine unfoldered. `_resolve_folder_id` (chat lane) matches the name case-insensitively against the live `/routine_folders` list (new connector `get_routine_folders`, tolerant of both the paged envelope and a bare list). Explicit `folder_id` wins; a miss creates the routine UNFOLDERED (folder_id=None, never NaN) and surfaces `created_unfoldered` (saved=True — the routine exists, so no retry/duplicate against the delete-less API) naming the missing folder. A folder-list read that raises degrades to unfoldered, never blocking the create.

**Status.** Code + tests complete; non-migration, self-merges on green per § Merge disposition. Implements a chat-ratified fix (the both-floors + folder-bundle + verify-don't-patch calls were put to Luke and confirmed).

**How you know.** `backend/tests/test_hevy_routine_numeric_and_folder.py`: NaN weight raises `RoutineNumericError` naming weight_kg + set 1 + the exercise, with NO GET/POST sent; ±Infinity, a string, and a bool are all rejected (parametrised); non-finite rest_seconds/superset_id raise; `0` stays valid; finite values reach the wire with no `NaN` token; `get_routine_folders` tolerates envelope + bare list; chat folder name → id (case-insensitive); folder miss → `created_unfoldered` saved=True, folder_id None; chat NaN → `invalid_number` (system_issue); REST NaN → 422. Full backend suite **1544 passed** minus the Py3.12 Garmin lane. The offline reproduction (json.loads accepts NaN → builder passes → httpx emits `NaN`) confirmed the mechanism before the fix.

**Do not revisit unless.** Hevy starts accepting non-finite values (it won't); or the numeric floor should extend to the workout READ/parse path (`load_events` — Q147); or `parse_constant` token-rejection is wanted as an extra ingress layer; or folder resolution needs to become create-or-resolve (auto-create a missing folder) rather than unfoldered-fallback.

### 290. Region→exercise consumer — loose mode-router

**Context.** `select_next` prescribes at region×capacity×side but never named a concrete exercise — the chat model improvised one per session, the root of exercise-duplication and the 55-routine sprawl. Prior reads established the substrate exists (`exercise_region_tags` M2M, `taxonomy.py`, `capability_observations`/`Measure` layer) but the **prescribe direction** (region_key+side → exercise) had never been built; the tag flow was one-way (logged exercise → region, `infer_loaded_regions`). A screen-vs-train distinction was confirmed absent (queue_eligible is a probe-queue filter, not a mode branch; `probe_block` is a training probe, self-disclaimed "not a screen").

**Decision — a downstream consumer, routed on tags-and-measure, NOT `queue_eligible`.** New `backend/engine/region_exercise.py`, pure-downstream of `select_next` (never mutates the prescription). Routing axis [REV-2 correction, Luke-ratified after Code disproved the brief's premise that §E is `queue_eligible=False` — only §G is]: classify each handed region by two orthogonal facts —
- has `exercise_region_tags`? → **TRAIN**: query region_key → templates (`_visible_to`-scoped, the resolver's own predicate #83), filter laterality-vs-side (a sided prescription needs unilateral/alternating; a bilateral template can't isolate a side — #287 two-sided-entry pattern), pick one by rotation/recency-avoidance.
- no tags, declares a `Region.measure`? → **SCREEN** via the Measure layer (§E: hop/decel/COD — measures exist, `record_observation` accepts `self_report`).
- no tags, no measure? → **SCREEN** honest stub (§G: "no instrument yet" — never silent, never a fabricated measure).
- target not a taxonomy region (a `fortify` free-text `primary_target`) → honest `target_not_a_region`, never a guess.

**Tiered tag usability [Luke-ratified].** `source='human_confirmed'` = TRUSTED → normal selection; `llm_proposed` = usable-but-PROVISIONAL → the exercise is still returned but `provisional=True` AND the `reason` string says so (the guess is surfaced in words, not a bare boolean a narrator can drop — the #283/#286/#288 acknowledgement discipline); no tags = honest fallback. Ships live off the `llm_proposed` reference and degrades toward trusted as tags are confirmed. Trusted candidates are preferred over provisional within a region.

**Loose, not lifecycle [Luke-ratified].** Per-call mode selection from current state. NO auto-promotion screen→train, NO persisted per-region mode machine, NO progression-tier selection — all DEFERRED (Q148). Rotation is the documented hook the tiering will extend.

**Scope honesty.** The consumer guarantees honest-not-silent for any region handed to it. It does NOT make the engine proactively surface §G: `compute_probe_queue` (untouched) never emits a `queue_eligible=False` region, so §G reaches the consumer only as a `fortify` target. Proactive §G screening is an upstream `compute_probe_queue` change, DEFERRED (Q148c). Fortify targets carry no side → default bilateral (the one V2 seam). Equipment is still not ingested (#285), so selection is not equipment-aware.

**Status.** Code + tests complete; non-migration (reads existing tables, no new schema), self-merges on green per § Merge disposition. Implements a chat-ratified brief; the one judgment Code exercised — the REV-2 routing-axis correction — was put to Luke and ratified before build, so no un-ratified call is embedded.

**How you know.** `backend/tests/test_region_exercise_consumer.py` (14 tests): trusted tag → concrete side-correct exercise; a sided prescription picks unilateral over bilateral, and bilateral-only tags → honest `no_side_match`; `llm_proposed` → exercise flagged provisional with the flag SURFACED in the reason (not just the field); trusted preferred over provisional; §E (measure, no tags) → `screen_measure` NOT train-fallback (the REV-2 regression guard); §G `sit_to_rise` → `screen_no_instrument`, never silent; the 3 needs_norm queue-eligible regions land sensibly (hip_ir_er → no-instrument, frontal_single_leg_stability → measure, a tagged aerobic_base → train); rotation avoids the recently-used exercise; `recent_template_ids` reads `hevy_sets` (DB-only); a non-region fortify target is honest; `resolve_prescription` handles probe+fortify (bilateral default) and tolerates missing blocks; the consumer never mutates `engine_out`. Wired live into `GET /engine/next` (adds an `exercise` key, prescription unchanged). Full backend suite **1558 passed** minus the Py3.12 Garmin lane (5 files import `garminconnect`, unavailable on the 3.11 verify sandbox; none touch this lane).

**Do not revisit unless.** Q148 is picked up (progression-tiered screen→train lifecycle; declaring §G measures so §G screens via the Measure layer instead of the stub; proactive §G surfacing via `compute_probe_queue`); or the tag-trust policy changes (currently `llm_proposed` is usable-provisional — if prod confirms tags, more of the pool becomes trusted with no code change); or equipment ingestion (#285) lands and selection should become equipment-aware.

### 291. HRV consumption rewire — live readiness/context surfaces read canonical_hrv (Garmin-aware)

**Context.** #265/#266 built the Q130 ingestion+unification half (Samsung dual-write into `hrv_readings`, held backfill, an ADDITIVE `/recovery/summary` `hrv` block) but left the LIVE readiness/context surfaces — the AM check-in passive snapshot, the prefill/current_state HRV baseline, and the MCP readiness tool — still querying `SamsungHRVReading.hrv_ms` directly. So Garmin HRV was stored and arbitrated but never reached the readiness signal a connected Garmin should drive. This is the deferred consumption half of Q130 (distinct from Q134's frontend-gated `/recovery/summary` restructure).

**Decision — surgical, scalar-only swap through `canonical_hrv`.** Each live surface now reads the HRV NUMBER through `reads.recovery_reads.canonical_hrv` (source-agnostic over `hrv_readings`, Garmin-arbitrated via `_SOURCE_RANK`, garmin=2 > samsung=1), not `samsung_hrv_readings`. The one invariant held: `hrv_readings` carries HRV ONLY (per the `HrvReading` docstring — it is deliberately not a generalisation of `samsung_hrv_readings`, which conflates HRV with sleep architecture). Every sleep / SpO2 / respiratory / architecture field STAYS on `SamsungHRVReading` (Garmin supplies none); where one Samsung row yielded both, the read is split — HRV from canonical, the rest from Samsung. No schema change. The old `context != 'session'` guard needs no analogue: every `hrv_readings` row is passive-overnight-equivalent by construction (the Samsung mirror early-returns unless `context=='passive_overnight'`; the Garmin connector writes nightly HRV only).

**API change (blocks the snapshot).** `canonical_hrv` gains `as_of: date | None` — an upper bound (`captured_at <= as_of`) applied BEFORE arbitration so same-night groups stay complete. `_snapshot_passive` needs "latest canonical at or before `for_date`", which the `since`-only (lower-bound) signature could not express.

**Rewired.** `checkin_v2._snapshot_passive` (→ `daily_records.passive_hrv_ms` → `/series/readiness`; HC sleep read unchanged) · `checkin_v2.get_prefill` rolling 7-night baseline (diary prefill left on Samsung — it reads bedtime/wake_time, sleep) · `current_state` `HRVBaseline` (the baseline the engine/chat see) · `mcp_server.get_readiness_snapshot` HRV figure (sleep/SpO2 stay Samsung).

**Deliberately NOT rewired — the device-readout rule [Luke-ratified this session].** A device-readout surface keeps device attribution; redirecting the HRV scalar into a Samsung-labelled readout would print a Garmin number under a Samsung banner on a contested night — a correctness bug in a readout, not a cosmetic call. So `/health/summary` + `/health/analyse-session` (the latter is session-scoped and Garmin has no session HRV), `context_builder._section_samsung_hrv`, and `mcp_server.get_recovery_metrics` stay wholly on `SamsungHRVReading`. `recovery.py` stays unmounted (no consumer). Both scope calls were put to Luke and ratified before landing (→ Q151 forces the recovery.py adopt-or-delete later; the multi-device readout that shows per-device rows is a separate future ticket).

**Forward-only caveat.** `/series/readiness` reads `daily_records.passive_hrv_ms`, frozen at AM capture. This rewire fixes FUTURE check-ins only; historical rows keep their Samsung-snapshotted values, leaving a seam at the Garmin cutover. A backfill recomputing `passive_hrv_ms` from `canonical_hrv` as-of each past date is a no-op until Garmin has history → deferred (Q152).

**Status.** Code + tests complete; non-migration (no new columns, reads existing tables), self-merges on green per § Merge disposition. Implements a chat-ratified brief; the two scope judgments (health.py, recovery mount) were put to Luke and ratified before landing — no un-ratified call embedded.

**How you know.** Full backend suite **1606 passed** on Python 3.12 (the CI substrate; the Garmin lane needs ≥3.12). New coverage: `test_garmin_hrv` — `as_of` upper-bounds AND keeps a boundary contested night's full group so arbitration stays complete + deterministic (Garmin wins), `since`+`as_of` window together. `test_hrv_consumption` — `_snapshot_passive` returns Garmin RMSSD on a Garmin-only night, Garmin on a contested night, Samsung on a Samsung-only night, latest-at-or-before-`for_date` (a later night does not leak into an earlier snapshot), and a Garmin `hrv_readings` row propagates end-to-end through `submit_am` into `daily_records.passive_hrv_ms`. `test_current_state` parity guard adjusted BY FIXTURE (mirror each Samsung night into `hrv_readings`, as the dual-write does) so the canonical baseline equals the pre-#43 Samsung-sourced one — the guard keeps exercising the baseline block rather than narrowing it out.

**Do not revisit unless.** Q152 is picked up (historical `passive_hrv_ms` backfill once Garmin has a stretch of history worth reflecting); or Q151 resolves recovery.py (adopt with a consumer, or delete); or the device-readout rule changes (the multi-device readout ticket makes `/health/*` / `_section_samsung_hrv` / `get_recovery_metrics` show per-device rows instead of a single scalar); or `_SOURCE_RANK` is reworked (Q131 — still unexercised, per-user overlap nil today).

### 292. HRV consumption = per-source-normalised deviation + cross-source confidence — supersedes `_SOURCE_RANK` arbitration

**Context.** #291 wired the live readiness/context surfaces to `canonical_hrv`, whose `_SOURCE_RANK` arbitration picks ONE source per night ("Garmin outranks Samsung"). That is the wrong end-state: two devices measuring the same night are two instruments, not two candidates for one truth, and arbitration discards the cross-source divergence that is itself signal. The offset between sources is non-constant (measured 0–48 ms across 5 dual-wear deload nights), so a single winner — or, worse, a raw-ms blend — throws away real information. This entry supersedes arbitration as the consumption model.

**Decision — a deviation+confidence reader (`reads.recovery_reads.hrv_deviation`).** Normalise each source to its OWN rolling baseline (z-score), combine the per-source deviations by weight, and surface cross-source (dis)agreement as CONFIDENCE, never as a hidden loser. Never blend raw values. Output is an object (`combined_z`, `direction`, `confidence`, `baseline_state`, `sources[]`, `agreement_gap`, `n_contributing`), read by downstream — never a bare scalar. Algorithm: per-source trailing baseline over `BASELINE_WINDOW` (mean + population SD, SD floored at `MIN_BASELINE_SD`); per-source `z = (today − mean)/sd`; per-night weight `= coverage × recency × baseline_maturity` where `maturity = min(1, baseline_n/MIN_BASELINE_N)`; `combined_z = Σ(w·z)/Σw`. A freshly-connected (thin-baseline) source therefore contributes at LOW weight until mature — it cannot flip an established source on day one (tested). Direction is up/down/flat about `FLAT_THRESHOLD`; confidence reflects maturity + agreement (high/medium on ≥2 mature sources by `agreement_gap`; low when they agree on direction but not magnitude / <2 mature; conflicted on opposite signs outside the dead-zone, with NO directional verdict; medium_low for a single mature source; very_low for a single immature source / no data; flat — confident-NEUTRAL — when all sources sit in the dead-zone). A recent phase change (`phase_change_date` within `SETTLING_NIGHTS`) caps confidence at low and sets `baseline_state="settling"` while still emitting the deviation (settling nights are IN the baseline — flag the verdict, not the input).

**Constants are PROVISIONAL (n=5-deload-informed) and calibration-gated on ~3–4 wk representative-load dual-wear.** All seven are named module constants, parameterised as keyword args (never hard-coded at the call site): `BASELINE_WINDOW`=28d, `MIN_BASELINE_N`=21, `MIN_BASELINE_SD`=3.0 ms, `SETTLING_NIGHTS`=10, `FLAT_THRESHOLD`=0.5σ, `AGREEMENT_HIGH`=1.0σ, `AGREEMENT_MED`=2.0σ. `AGREEMENT_HIGH`/`AGREEMENT_MED` are the primary calibration targets.

**Confidence value set — one flagged interpretation for chat.** The brief's output-contract line listed five tiers (high/medium/low/very_low/conflicted); the brief's confidence TABLE additionally specifies `flat` (confident-neutral) and single-mature `medium_low`, and its critical-semantic test REQUIRES an all-flat night to be confident (never very_low, never low). Rather than fold those two into a tier that would misstate them ("nothing's happening" ≠ "we don't know"), the reader emits the full table faithfully — the confidence set is {high, medium, medium_low, low, very_low, conflicted, flat}. This is disambiguation of the brief's own table, not a new call; flagged here and in the PR so chat can re-map if the five-value listing was meant as closed.

**Consumer migration — all-or-nothing (the four #291 paths), each assigned explicitly.** A partial cut would leave arbitrated HRV on one surface and deviation on another — self-contradiction, the exact failure dual-reader risks. All four now derive from ONE model (`hrv_deviation`), so no surface disagrees with another: (a) `checkin_v2._snapshot_passive` → `daily_records.passive_hrv_ms` (→ `/series/readiness`): the single ms scalar = `representative_source(...)["rmssd"]` (highest-weight source); (b) `checkin_v2.get_prefill` `hrv_vs_baseline`: the representative source's `rmssd − baseline_mean` (its own baseline), same model as (a) so the two scalars compare like with like; (c) `current_state.hrv_baseline` (was `hrv_baseline_7d`): the representative source's OWN rolling baseline mean/n/latest/diff — no longer a 7-night mean of `.canonical` rows, so the field and its chat-context label ("HRV baseline (rolling)", was "7-day HRV baseline") were corrected for honesty; (d) `mcp_server.get_readiness_snapshot`: a rich LLM readout (NOT the device-attributed `get_recovery_metrics`, which stays wholly Samsung), so it surfaces BOTH the representative ms figure and the full deviation object (combined z, direction, cross-source confidence).

**Not migrated — `recovery.py` (unmounted, Q151).** It is the SOLE remaining caller of the arbitration selection (`.canonical`). Its disposition is Q151 (adopt-with-a-consumer or delete); leaving it is deliberate — this task did NOT manufacture a second unconsumed reader (cf. Q151 itself). Aerobic arbitration (`aerobic_reads._SOURCE_RANK`, `load_events_metabolic`) is a DISTINCT concern and untouched.

**The exit — dual-reader is a transition with a defined end (Part 3, tracked Q153).** Fine cut: the source-tagged row-reading plumbing (`_hrv_rows`) is shared and KEPT; only the selection layer (`_SOURCE_RANK`, `_rank`, `_win_key`, `arbitrate`, the `.canonical` flag) is superseded. Once no consumer calls the arbitration selection — today only unmounted recovery.py does — delete `_SOURCE_RANK` and the arbitration branch, keep `_hrv_rows`. If a future single arbitrated number is needed, derive it from the deviation model via `representative_source` (highest weight), NEVER a resurrected rank. Until deletion the arbitration branch stays callable but NO NEW CONSUMER may call it — enforced by review + the reader's docstring.

**Status.** Code + tests complete; non-migration (no schema change, reads existing tables), self-merges on green per § Merge disposition. Implements the chat-ratified #292 brief; the one disambiguation (confidence value set) is flagged above, not a new architectural call — no un-ratified decision embedded.
