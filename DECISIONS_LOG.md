...
### #NEXT. v1 definition of done adopted; surfacing-phase sequence set; co-equal-modules principle amended to sequence-by-consumer
...
**Status.** ADOPTED (governance-only: ROADMAP + `closeout.md` + the `/closeout` ritual + this entry). Two further concerns the brief named for demotion — the Banister dosing wire (`selection.py` ← `load_metrics`) and further psychological-window work (Q122) — have **no standalone ROADMAP row to move** and are left for operator disposition (a Q may not be relocated); the demotion list and these two were flagged for Luke's confirm/edit at handoff per the brief. Held for ratification, not self-merged — the v1 definition and the amended principle are new strategic judgments, not a no-new-judgment brief.

**How you know.** `ROADMAP.md` now opens with `## v1 — definition of done` and `## Surfacing phase — sequence` before NOW; the Appointment brief NOW row carries "sequence position 3"; the three demoted rows sit in LATER with a `DEMOTED (2026-09-09)` stamp and their notes preserved; LATER, User rollout, and Dependencies/blockers were rewritten with per-row citations (User rollout found chat's "Georgia on Polar H10" unsupported in-tree — Q98 records three accounts, every Polar/H10 ref is Luke's); `closeout.md` and `.claude/commands/closeout.md` gained the "which v1 test does this lane serve?" handoff prompt. All on `claude/roadmap-v1-steer-i5l54q`.
