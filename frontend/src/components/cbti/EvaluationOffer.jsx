...
//
// A `close` recommendation renders WITHOUT an accept control: block close is
// engine-driven (#118) and has no built path, so the surface says so rather than
    ...
    <div className="bg-white border-2 border-indigo-200 rounded-2xl p-4 mb-4 text-left space-y-3">
      ...
      {ev.acceptable ? (
        ...
        <p className="text-[11px] text-gray-500 leading-snug">
          The engine recommends closing this block. Block close is engine-driven and is not
          minted here — no accept control is offered.
        </p>
      )}
    </div>
  )
