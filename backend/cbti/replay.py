...
class LiveCycleEval:
    ...
    is the engine's and may still be a HOLD (insufficient nights, adherence).
    """
    eligible: bool
...
def evaluate_live_cycle(db, user_id: int, block, today: date) -> LiveCycleEval | None:
    ...
    across every prior cycle in the block, and the TST-plateau exit needs two of them.
    Evaluating the live cycle alone would start that history empty, so `close` could
    never fire and the block could only ever be extended or held — the engine would be
    structurally unable to recommend the one decision that ends a block.

    ELIGIBILITY IS CALENDAR DAYS, NOT LOGGED NIGHTS. #118 specifies "once >=7 days have
    elapsed since the current prescription's effective_from", and `replay()`'s own cycle
    spans are calendar-dated (`cycle_start + CYCLE_NIGHTS - 1`), so days is the unit that
    matches both. Gating on LOGGED nights instead would mean a single unlogged night
    defers the offer indefinitely — cycle 1 can never reach 7 logged nights once one of
    its 7 calendar days is missing — which silently strands the operator mid-titration.
    Night COUNT still governs the decision, but through the engine's own sufficiency gate
    (>= MIN_VALID_NIGHTS valid), which returns a HOLD naming the shortfall rather than
    withholding the offer. Both quantities are reported so the operator sees each.
    """
    ...
    # Cycles belonging to the live prescription whose 7-day span has fully elapsed. A
    # partial trailing cycle is excluded: offering a decision over 3 nights because the
    # operator opened PM on day 10 would adjudicate an unfinished cycle.
