...
def test_rpe_usable_but_no_e1rm_uses_half():
    sl = compute_set_load({"weight_kg": 100.0, "reps": 5, "rpe": 8.0}, rpe_usable=True, e1rm=None)
...
def test_rpe_absent_rep_set_bands_by_reps():
    """No usable RPE → Mechanical m=1.0, NM from the reps-band prior (no h(I))."""
    sl = compute_set_load({"weight_kg": 100.0, "reps": 5}, rpe_usable=True, e1rm=125.0)
...
def test_reps_band_prior_tiers():
    assert compute_set_load({"weight_kg": 50.0, "reps": 5}, rpe_usable=False, e1rm=None).neuromuscular == pytest.approx(0.6)
    assert compute_set_load({"weight_kg": 50.0, "reps": 8}, rpe_usable=False, e1rm=None).neuromuscular == pytest.approx(0.35)
    assert compute_set_load({"weight_kg": 50.0, "reps": 12}, rpe_usable=False, e1rm=None).neuromuscular == pytest.approx(0.15)


def test_warmup_half_mechanical_zero_nm():
    sl = compute_set_load({"type": "warmup", "weight_kg": 60.0, "reps": 5, "rpe": 5.0},
                          rpe_usable=True, e1rm=100.0)
...
def test_non_rep_distance_bridged_nm_zero():
    sl = compute_set_load({"weight_kg": 40.0, "distance_meters": 20.0}, rpe_usable=True, e1rm=None)
...
def test_non_rep_timed_hold_bodyweight():
    sl = compute_set_load({"duration_seconds": 60}, rpe_usable=True, e1rm=None)
...
def test_bodyweight_rep_set_uses_operator_weight():
    sl = compute_set_load({"reps": 10, "rpe": 9.0}, rpe_usable=True, e1rm=None)
...
def test_empty_set_skipped():
    assert compute_set_load({"type": "normal"}, rpe_usable=True, e1rm=None).skip is True
...
# ── Session-level: epoch gating + laterality (mutation-proofed) ────────────────
...
def test_epoch_gating_flips_banding():
    """Same set post- vs pre-epoch: post uses (reps,RPE) banding, pre bands by reps."""
    ...
    pre = compute_session_events(_sess(date(2026, 5, 1), [(0, "T", st)]),
                                 laterality_by_template={}, e1rm_by_template={"T": None})
    assert post["mechanical"]["load"] == pytest.approx(575.0)   # m(RIR2)=1.15
    assert pre["mechanical"]["load"] == pytest.approx(500.0)    # RPE not trusted → m=1.0
    assert post["neuromuscular"]["provenance"]["rpe_sets"] == 1
    assert pre["neuromuscular"]["provenance"]["reps_banded_sets"] == 1


def test_unilateral_pair_halved():
    ...
    # two blocks of 50*10*1.0=500 each → 1000, halved → 500
    assert ev["mechanical"]["load"] == pytest.approx(500.0)
    assert ev["neuromuscular"]["load"] == pytest.approx(0.35)   # (0.35+0.35)/2
    assert ev["mechanical"]["provenance"]["halved_templates"] == ["UNI"]


def test_untagged_pair_indeterminate_not_halved():
    """Mutation-proof: identical structure, untagged → surfaced, NOT halved."""
    st = {"weight_kg": 50.0, "reps": 10}
    sets = [(0, "UNT", st), (1, "UNT", st)]
    ev = compute_session_events(_sess(date(2026, 6, 1), sets),
                                laterality_by_template={},        # untagged
                                e1rm_by_template={"UNT": None})
    assert ev["mechanical"]["load"] == pytest.approx(1000.0)     # NOT halved
    assert ev["neuromuscular"]["provenance"]["indeterminate_laterality"] == ["UNT"]
    assert ev["mechanical"]["provenance"]["halved_templates"] == []


def test_bilateral_pair_not_halved():
    st = {"weight_kg": 50.0, "reps": 10}
