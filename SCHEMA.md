...
**`daily_records.passive_hrv_ms` — retained, read-only, no longer written** (HRV staleness, `#NEXT`; data migration `e3b7c5a1f942`, no schema change). The AM Save stopped denormalising HRV: every surface reads current-day HRV live from `hrv_readings` by wake-day equality, and historical readers (`/series/readiness`, MCP `get_checkin_history`) fall back to this column **only** for a day with no value-bearing `hrv_readings` row. `e3b7c5a1f942` NULLs the user-1 rows that froze a prior night (a forward carry: no same-day `hrv_readings` row with that value; the most recent earlier row with it is 1–2 days back) — it self-verifies the count (`EXPECTED_COUNT = 12`) and refuses on mismatch. Irreversible (downgrade is a no-op; the values are printed in the upgrade log).
