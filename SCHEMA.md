...
**`daily_records.passive_hrv_ms` — retained, read-only, no longer written** (HRV staleness, `#327`; data migration `e3b7c5a1f942`, no schema change). The AM Save stopped denormalising HRV: every surface reads current-day HRV live from `hrv_readings` by wake-day equality, and historical readers (`/series/readiness`, MCP `get_checkin_history`) fall back to this column **only** for a day with no value-bearing `hrv_readings` row. `e3b7c5a1f942` NULLs the user-1 rows that froze a prior night (a forward carry: no same-day `hrv_readings` row with that value, and an earlier row with exactly that value at carry age `>= 1` day, no upper bound). It self-verifies against `EXPECTED_CARRY_COUNT`, an operator placeholder set from the post-deploy preview, and refuses with zero writes when that is unset or mismatched. Irreversible (downgrade is a no-op; the values are printed in the upgrade log).
...
| `time_range` | str | no | display + read by the overlap check (#281): "HH:MM-HH:MM" or a lone "HH:MM" resolves the occupancy interval, overriding the `time_of_day` band |
...
| `event_date` | date \| null | no | a DATED one-off (#317/Q165) — ISO; MUTUALLY EXCLUSIVE with `days`/`sessions_per_week` (a commitment names calendar dates OR weekday recurrence, never both). The planner marks the day(s) unavailable (heavy → next-day caution) and ignores it once past; a dated item has no `days`/`satisfies`, so it counts 0 toward any quota and never collides on the day-based overlap check. |
| `event_end` | date \| null | no | optional end of a date-range one-off; on or after `event_date`; requires `event_date` |
...
**`satisfies` — the schedule↔quota link (#312).** Optional; absent or null = *unlinked*, exactly as before. When present it is an object with **exactly one key**, one of the three microcycle slot kinds: `{"capacity": <token>}` (a movement-quality token — `engine.taxonomy`), `{"load_window": "metabolic"}` (`load_events_metabolic.WINDOW_METABOLIC`), or `{"activity": <name>}` (#315 — a declared device-evidenced activity, an OPEN-vocabulary non-empty string, matched to a declared `activity` slot at read). Capacity/load_window are validated against the SAME closed vocabularies the microcycle slot validator uses (imported, one definition); `activity` is open. The link is read on the chat-context read path only — `context_builder._section_training_phase` derives a per-slot **scheduled vs quota vs done** consistency line from it (MISMATCH/UNPLACED claimed only on a 7-day leg); nothing writes back and no engine selection depends on it.
...
**The overlap rule — triggers on day overlap AND time overlap (#281, refines #233).** A write that lands on a day any active row of the same user already holds **and whose clock time overlaps that row's** is refused **409**, regardless of `activity`, naming every colliding row (id, activity, days, time_of_day). The retry must acknowledge *every* named row via `supersedes: <id>` (replaces it) or `distinct_from: [<id>, …]` (a separate commitment sharing a day and time). Time is resolved per row (`_time_interval`) to a `[start, end)` interval: a parseable `time_range` wins (else the `time_of_day` band — morning 05:00–12:00, afternoon 12:00–17:00, evening 17:00–22:00). **Non-overlapping same-day times now coexist automatically** — work(morning) and gym(16:30) save with no acknowledgement, the decompression-schedule symptom #281 fixes. **Unresolvable time** (`time_of_day` "unknown"/absent with no parseable `time_range`, or a present-but-unparseable range) is treated as possibly-overlapping, so the ambiguous case stays visible rather than silently double-booking. #233's day-alone trigger is superseded, but its guarantee survives: a true duplicate (a new key for an existing commitment) shares that commitment's time and still collides. Matching on `activity` stays rejected — string equality over LLM free text fails **open**. The `unknown field(s)` refusal also advertises the write-only `distinct_from` (so it never contradicts the overlap advice) and redirects a mis-nested `active` to the top-level deactivation lever.
...
**Writers — three exits, not interchangeable (#269).** A row leaves the active set by exactly one of three writers, each with a distinct meaning: **supersession** (`upsert_knowledge_entry`, via the `supersedes` / day-overlap path — replaced by a newer statement, names its successor in `superseded_by`); **expiry** (`POST /knowledge/expire-stale` — the overlay sweep for rows whose predicted `expires_at` has passed; writes no `resolution` block); and **resolution** — `POST /knowledge/schedule/{entry_id}/resolve` (`routers/knowledge.py::resolve_schedule_item` → the shared `_resolve_entry` body it shares with the injury route). Resolution is an explicit operator assertion that a live commitment is retired: it requires a non-blank `basis` and a `resolved_by` in `{user, clinician}`, adds a `resolution` block to `value` (a reassignment — the column is plain `json`, not a `MutableDict`), flips `active=False`, and leaves `superseded_by` untouched (a resolution has no successor). It is scoped to `type='schedule_item'`, so an injury id through this route is a 404 that leaks nothing — the routes are per-type by design and are not generalised to `/entry/{id}/resolve`. Resolving an already-inactive row is a 409.
...
**Pre-existing rows were backfilled 2026-09-08 (`OPEN_QUESTIONS` Q116, DONE; GATE 4 satisfied).** The validator has been live at write since `#233`; the legacy active rows were then corrected against production (operator via `railway ssh`): the live-row stop-condition passed, then a single-commit backfill of 22 changes in **retire mode** (season_end 2026-09-05 having passed, three season rows were corrected for history then retired). End-state is **12 active rows, all conforming** — prose moved out of `same_day_training` into `same_day_note`, quota values out of `days[]` into `sessions_per_week`, the `minimum_days` key removed, duplicate pairs split-then-retired, stale-active rows retired. Legacy nonconformance now survives **only on inactive history rows**, untouched by design (validation is at write; inactive rows are never re-read by the calendar path). The five prod assertions were executed and passed.
...
**Validated at write, stored verbatim.** `engine/training_phase.validate_training_phase` refuses (422) an unknown/duplicate capacity, a bad `probe_posture`/`asserted_by`/`source`, a future or non-monotonic `entered_on`, a `review_on` before `entered_on`, and a malformed `microcycle` (`sub_cycle_days` 3–28; `sub_cycles` 1–4; each slot carries `sessions_per_cycle` + `minutes` 5–180 and EXACTLY ONE kind key — `capacity` | `load_window` | `activity` (#307 → #315); a `load_window` OR `activity` slot REQUIRES a non-empty `device_sports` list (sport-scope — a recorded WALK can never satisfy a conditioning quota, #315), a `capacity` slot refuses it; duplicates checked **per-sub-cycle** for all three kinds so the same key across A/B is allowed). `device_sports` is an OPEN vocabulary (non-empty strings; Polar sports are free-form), matched to a canonical session's `sport_name` exact but case-insensitive at read. A slot may also carry `recorded_via` (#317 — optional, closed set `hevy | polar_h10 | garmin | samsung_health | manual`): how the slot's sessions are recorded, so the phase-change form can WARN when a choice has no evidence path (Q159); metadata to the resolver/planner, form-written only. `capacities`/`microcycle` are byte-identical PUT/GET. Router: `POST /engine/phase` (open), `POST /engine/phase/close` (→ 404 if none open), `GET /engine/phase` (current | null, with `review_due`), `GET /engine/phase/history`.
...
No migration. Two #314 conventions recorded for discoverability:
...
ALTER TABLE health_connect_syncs ADD COLUMN sleep_end                  TIMESTAMPTZ;  -- latest segment edge of the MAIN period
ALTER TABLE health_connect_syncs ADD COLUMN sleep_onset                TIMESTAMPTZ;  -- first ASLEEP (LIGHT/DEEP/REM) stage from a REAL stage record
ALTER TABLE health_connect_syncs ADD COLUMN sleep_start_source_package TEXT;         -- writer of the segment supplying sleep_start
ALTER TABLE health_connect_syncs ADD COLUMN sleep_end_source_package   TEXT;         -- writer of the segment supplying sleep_end
```

- **Main period.** This is the same period the duration comes from (#254/#256): every session ending on the wake-day, from every writer, is split into stage segments, grouped by overlap or adjacency, and the period with the most asleep time is taken. The clocks and the duration can therefore never describe different sleep.
