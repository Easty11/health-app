...
| 21 | 2026-08-27 | `#243`'s closing-figures block (PR #115) asserted "`#245` = 0 / all `bw_fraction` NULL at recompute time", inferred from `#245`'s closeout ("tagging pass not yet done") without a prod read. The tagging pass had gone live before the 27 Aug recompute; the operator's reversible prod experiment (nulling Push Up `392887AA` raised Mechanical +821.1, restore reverted exactly) and `audit --strict` exit 0 measured the opposite. Corrected by `#247`. | MODEL | `#245` closeout ("tagging not yet done", true when written) → current prod `bw_fraction` state at the 27 Aug recompute | UNSIGNED | — | — | Measure a prod figure or state before asserting it; never inherit it from a closeout or adjacent doc — a closeout is point-in-time and goes stale by the next recompute. §18 (state inferred from an adjacent attestation is not measured state) applied to prod state; no new rule minted. | STANDS · corrected in DECISIONS_LOG #247 |
