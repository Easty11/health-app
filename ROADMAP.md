...
- **Trigger resolution settled (#NEXT):** the endpoint resolves the trigger as the newest `collected_date` (a draw), not a single report — the seven-reports-one-date panel falsified the report-shaped spec. Recorded so the endpoint inherits it; no endpoint built yet.
