...
| **Guard:** a test asserting SCHEMA.md does not lag `models.py` | **OWED.** SCHEMA.md is repo-canonical and its never-lag rule currently has **no enforcement** — it is aspirational. Four columns from migration `c4e8a2019bd7` (`basis_n_samsung`, `basis_n_diary`, `basis_n_alcohol_unknown`, `basis_tib_over_run_min`) sat in `models.py` and absent from SCHEMA.md until a VERIFY happened to look; that was an omission when the migration was folded in, and folds will recur. The detector already exists and was demonstrated working: compare each CBT-I column name in `models.py` against SCHEMA.md, **with a negative control** — a fabricated column name must report LAG, proving the check can detect absence rather than always passing. Currently used once and discarded. As a test it makes the rule enforced rather than aspirational. Separate concern from CBT-I surfaces, so not that branch. Owner: Luke. Recorded here per #112 — it exists only in conversation otherwise, which is the failure Q45 and the co-occurrence test both taught. |
