...
| 4b-i | **Producer structural half.** Three-pass restructure (`should_surface` computed after relations), `relations_rendered` with operand degradation, `meta.protocol_context_snapshot` dated to the panel. No demotion, no levers, no verdict, no endpoint. | **OWED** — on `feat/interpretation-relations`, pushed, held for review (unmerged). Backend suite 453. |
| 4b-ii | **Producer interpretive half.** Relation-based demotion of gate 1's delta arm, `shared_levers` with already-in-play filtering, `axis_verdict`, `mechanism` / `stable_rationale`, `expected_by_phase`, the endpoint, and the view wired fixture→live. Two forks decided here: cache-on-confirm vs compute-on-read, and verdict depth. | **BLOCKED** — see below |
...
**4b-ii's blockers — all attach to 4b-ii only; 4b-i is unblocked, which is why it was separable and has already landed on `feat/interpretation-relations`:**

- ~~**Safety-band citation capture.** The safety-threshold asset is empty (`thresholds: {}`), so gate 3 resolves undecidable (`no_asset`) for every marker and the news gate's safety arm is inert with it.~~ **Discharged by #139** — the three haematocrit bands are live in `thresholds` with per-band `evidence_refs`; gate 3 fires for haematocrit and gate 1's safety arm is reachable. Struck, not deleted (correct-don't-delete). No longer a blocker.
- **Ingestion.** The lab store is empty — `lab_reports` and `lab_results` both hold zero rows. Blocks **4b-ii's view-wiring half only**; the producer library is testable against the fixture with an empty store, which is what 4b-i exercised. The early-Aug TRT panel named as the first real consumer above is the natural first input, and also the pre-dose-increase prior the next panel will be differenced against.
- **Phase-vocabulary mismatch (new).** `marker_groups.json` gates `feedback` relations on `precondition_phase: "on_trt"`; `derive_phase` returns `steady|episodic|washout|stopped|re_entering|None`. No mapping exists in asset or code, so no `feedback` relation can be evaluated — 4b-i emits them `unresolvable`. Asset content; blocks 4b-ii demotion of the feedback arm. `OPEN_QUESTIONS` Q-NEXT. Owner: Luke.
- **Lever→declared-factor join (new).** Lever nodes carry no field linking them to declared-state factors (different namespaces, no mapping field), so I3 already-in-play filtering cannot be implemented. Smallest fix: an authored `declared_factor_keys: []` on each lever node. Asset content; blocks `shared_levers[]`. `OPEN_QUESTIONS` Q-NEXT. Owner: Luke.
