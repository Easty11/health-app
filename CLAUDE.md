...
- **Interpretation increment 3 — lever-tap → scoped ephemeral education thread, backend spine (`#268`, this PR)** - `interpretation/education_seed.py` (seed read from the producer, #49 lock) + `education_thread.py` (stateless, injected client, fail-closed output guard) + `POST /interpretation/education-thread`; `contains_directive` extracted as one shared #47 detector from the rephrase validator; three evals. Frontend tap surface deferred → Q139. Handoff: `closeout.md`.
