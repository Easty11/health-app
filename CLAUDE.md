...
- **Load refresh triggered on demand per-user + a nightly in-process sweep — supersedes #296's dedicated Railway cron service (#297) — the #296 orchestrator (`scripts/refresh_load.py`) is unchanged; only its trigger moves. `POST /load/refresh` (`routers/load.py`) runs the chain for the calling user, staleness-gated on `max(LoadMetric.computed_at)` at 15 min (`force=true` bypasses); a background task in `main.lifespan` (`load_sweep.py`) sweeps all users at 02:00 Brisbane and once on startup if stale >24h — both off the loop via `asyncio.to_thread` (the chain uses `asyncio.run`). Frontend (operator-ratified BOTH): Metrics gets the render-and-refresh loop over `/series/load`, Training fires it fire-and-forget on the post-workout landing, via a shared server-authoritative `useLoadRefresh`. `railway.cron.toml` deleted; no schema change. Code + frontend, self-merged on green (`#297`)** - Handoff: `closeout.md`.
