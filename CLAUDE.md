...
- **Cystatin C mapped to `cystatin_c`, stored row bound by the standing backfill (#217)** - the canonical entry lands and the `#55`-sibling backfill rider binds the single row stored unmapped from the 2026-08-04 SNP draw, so the marker trends and the next upload auto-maps at confirm. `unit_established` was taken from the STORED unit rather than the printed one, which is what keeps the §6 over-collapse guard quiet on the next upload instead of 422-ing a just-mapped marker. Unblocks Brief B (renal derived metrics), whose placement fork the same session closed by tracing `is_derived` - found write-dead in prod, with the unreachable stale-derived branch it implies raised as `Q103`. See DECISIONS_LOG #217.
