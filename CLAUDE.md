...
- **Interpretation go-live: first live run against the 2026-08-04 draw (#194)** - three reference assets + six levers promoted `ai_draft` -> `human_verified` on Luke's O2 content verification; stale `_deferred.groups.erythroid` removed (group already active and surfacing live); haematocrit 0.50 fires the safety arm at the band boundary carrying band + citations + contested, register education not urgency; #51 render-gate confirmed convention-only, not code (Q92). See DECISIONS_LOG #194.
- **Status reporting: parser gate + snapshot baseline — Steps 1–2 (#190–#193)** - repairs the live `**State:**` extraction bug in `gen_governance_view` (89 health-app questions rendered blank); centralises both repos' store grammar to `scripts/gov_dialects.py`; adds a gated machine status model `scripts/gen_status_model.py` (count-parity + sequence + non-empty-extraction gates, off-vocab/drift tally); seeds the first cross-repo snapshot to `Projects/_status/` (outside both repos, baseline-flagged + provenance-stamped). Opens Q90 (HCA question-vocab drift), Q91 (brief-authoring verify-checkpoint). See DECISIONS_LOG #190–#193.
