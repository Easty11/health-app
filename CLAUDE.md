...
- **CBT-I PM evaluation trigger, reworked onto the 4-night engine (#118 → #213)** - offer (`GET /checkin-v2/cbti/evaluation`) + witnessed accept surfaced in `NightlyCloseOut.jsx`, both through `evaluate_live_cycle` (#128 reuse); rebuilt fresh from master rather than merging the obsolete 7-night branch, so the block-close arm is deleted as unreachable (the hunting engine emits a `converged HOLD`, never `close`) and master's centre-estimate features are preserved rather than dropped. First product-code session after two governance/instrument ones. Merged via PR #66. See DECISIONS_LOG #213.
