...
- **Narrate-after-write on the chat knowledge/schedule lane (Q143a / WS4, builds on #283) — `/chat` can no longer narrate a write as saved when it wasn't. The reply is generated in ONE pass BEFORE writes run, so `_compose_response` branches on `write_results`: all saved → keep the pre-write reply, no second call; any failed → discard the prose and run a bounded pass-2 (`_narrate_after_write`, same client, `max_tokens=1024`) reporting the true per-row outcome, affordance TYPE-derived from `reason_code` (`day_time_clash` offers supersedes/distinct_from; a bug code is flagged as ours, never handed to the user). An always-on deterministic footer from `write_results` (`✓ N saved` / `⚠ X saved, Y failed — …`) is the hard floor. `context_builder` STEP 2 (parity-excised span): the model REQUESTS, the system REPORTS. Corrected the #283/Q143 grep miss — the web `ChatPanel` DOES consume `/chat` (`ChatPanel.jsx:78`; FEEDBACK §42). Cost: +1 call on failed-write turns only. Code, self-merged on green (`#284`)** - Handoff: `closeout.md`.
