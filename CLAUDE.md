...
- **Card sleep repointed from the frozen Samsung scraper table to the freshest staged night across sources (Health Connect + Samsung), source-labelled and dated (#300). `/health/summary` (`routers/health.py`) gains an additive `latest_sleep`: newest staged wake-date across `{samsung_hrv_readings, health_connect_syncs}` (same-night tie → the HC aggregate), source-labelled via a `health_connect_record_sources` join on `{night, night-1}` (record_start is the session START, normally the prior evening; `_aggregate_day` keys the sync on `_wake_date(endTime)`). Fields the winning source doesn't stage render `—`, never a stale value from another night (Decision A, Luke — for an HC/Garmin night: efficiency, awake/WASO, resp rate, sleep HR, SpO2, bedtime→wake); a ≥2-real-package night labels `Health Connect (multiple)`, never a misattributed device (Decision B; catches Samsung+Withings, Q83). The Samsung-native HRV path (`latest`/`latest_hrv`/baseline) is untouched, so the wake-day HRV selector brief is unaffected; makes the card agree with the check-in on sleep source. Read-path + frontend only; no ingestion, no scraper removal, no schema change. Code + frontend, self-merged on green (`#300`)** - Handoff: `closeout.md`.

- **`garmin_sync` gains a trigger — on-read freshness + a nightly guarantee, on #297's in-process rail (#299). `POST /integrations/garmin/refresh` (`routers/garmin.py`) runs a narrow Garmin HRV pull for the calling user on Recovery-card open, staleness-gated on `max(hrv_readings.created_at)` for `source='garmin'` at 30 min (`force=true` bypasses), never erroring the card (dead token / API failure returns last-good); and `garmin_sync` is added as the second per-user job in the existing 02:00 Brisbane sweep (`load_sweep._sweep`, via a non-CLI `scripts.garmin_sync.sweep_garmin_hrv`), beside the load chain, per-user isolated. On-read owns same-morning freshness (fires after Garmin's ~6am sync); 02:00 can only land the prior night, accepted as insurance. `HealthPanel` (Recovery card) fires the refresh on mount via `useRecoveryRefresh` (mirrors #297's `useLoadRefresh`) and re-fetches `/health/summary` on a real run; Refresh = force. No dedicated cron, no new scheduled time, no schema change. Resolves Q155 — the deferral #296/#297 both named. Code + frontend, self-merged on green (`#299`)** - Handoff: `closeout.md`.

- **Recovery card (`/health/summary`) + MCP `get_recovery_metrics` now show the newest HRV across sources (Garmin + Samsung), source-labelled — the multi-device readout ticket #291 anticipated, for these two surfaces (#298). `/health/summary` gains an additive `latest_hrv` (newest across sources, `source`-tagged; Samsung `latest`/sleep/baseline unchanged; a Garmin-only user now surfaces HRV); `get_recovery_metrics` unions samsung+garmin rows, each line labelled, both shown on a dual night (never collapsed, never silently preferring Samsung); `HealthPanel` headlines it with a source chip and never pins the Samsung ±baseline badge onto a Garmin number. Newest-wins, same-night tie → richer source (garmin); raw ms never blended (#292). The multi-source deviation model and `_section_samsung_hrv` are untouched. Diagnosis surfaced that Garmin HRV has no ingestion scheduler (Q155). Code + frontend, merged on green after live ratification — reverses the #291 device-readout clause for these two surfaces, so NOT self-merged (`#298`)** - Handoff: `closeout.md`.
