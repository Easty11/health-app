...
- **Phase-change form — fixes from the first real prod run (#320). The operator's Continue-with-revise wrote correctly (atomicity held every attempt) but took ~10 tries: B1 every placement shipped `time_of_day: 'unknown'` (the server rightly 422'd on every weekday) → the form now collects a resolvable time (bucket or start–end range, bucket derived; `deriveTimeOfDay`), never 'unknown' for a weekly placement; B2 the Confirm listed only what was SET → it is now a DIFF (removals first; `diffSlots`) and removing every capacity slot demands an extra tick; B3 step 5 ignored the existing schedule → it loads the active items (hard first, then soft) for keep/relink/retire and the live counter is the backend's own `consistency_rows` via read-only `POST /engine/phase/transition/preview`, never re-derived client-side. F17: the overlap 422 carries the colliding rows + offending key structurally through the transition; the form names the clash and offers "separate sessions" → `distinct_from` resubmit, verbatim server text under a disclosure. Also F1 (review note → `close_prior_reason`), F5/F6 (slot cards + replace-or-add on a changed prefill), F12 (`week_plan` availability/caution), F18 (per-step purpose). Draft read gains item `id`s + `week_plan`. GUARD: no change to the transition's atomicity, the validators, or `resolve()`; server stays the validator; the form never guesses a time. Backend + frontend + tests; non-schema. Code, drafted via harness PR (`#320`)** - Handoff: `closeout.md`.
