...
- **A FEEDBACK entry wrong in scope is amended in place; `§30` named only the read half (#229)** - `§30` diagnosed a real defect from a read-side symptom and closed with an instruction - never "fix" data that reads fine over HTTP elsewhere - which is correct for the read path and actively suppresses the write path, where the same missing `charset` makes PowerShell 5.1 downgrade the request body BEFORE IT IS SENT. Four operator writes to `POST /knowledge/entry` (ids 75-78) persisted their em-dashes as plain hyphens: loss AT REST, not display. A store read at session start was steering the next session away from a live defect. FEEDBACK IS AMENDED IN PLACE, NEVER SUPERSEDED (#229) - the opposite convention to `DECISIONS_LOG`, and deliberately so: a decision is a record of what was decided and when, so supersession preserves the reasoning trail; FEEDBACK is a live instruction set read at session start, so a wrong rule sitting above its correction is a rule that STILL FIRES, and its trail belongs in `FEEDBACK_ARCHIVE.md` and git history rather than in the instruction. First instance of the convention; the superseded text stays recoverable via `git log -p`. THE EM-DASHES ARE THE ONLY REASON IT WAS SURVIVABLE - an em-dash degrading to a hyphen is legible, but the channel's real traffic is lab and imaging notation (`µg/L`, `°`, `×`) where the same silent downgrade is not, and a value that reads plausibly after corruption is indistinguishable from one that was never corrupted. The `charset=utf-8` fix is NOT LANDED - it is gated behind a live `µ` round-trip, a prod verification per `§8`, not a repo write. Adds `§31` (auditing a vocabulary's members does not audit its coverage - the channel axis was audited in #227 and took four writes it has no member for one day later) and Q111-Q114. No code, no schema, no migration, no tests. See DECISIONS_LOG #229.
