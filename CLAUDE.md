...
- **CBT-I eval-trigger test tz-skew fix — anchor fixtures on the engine's AEST clock (test-only, PR #154), landed + merged** - `test_cbti_eval_trigger.py` seeded fixtures on naive `date.today()` (container-local, ≈UTC in CI) while the engine counts elapsed days against AEST (`_today_aest()` = `datetime.now(Australia/Brisbane).date()`), so the day-count and `successor.effective_from == today` assertions went off-by-one whenever the local date lagged the AEST date — reddening the suite on the calendar, not on any code change. Fix imports `_today_aest` from `routers.checkin_v2` and anchors every fixture date and the assertion on it (and drops the now-unused `date` import); test-only — no engine/`routers/` change, because AEST is the user's calendar and the engine is correct. Diagnosis confirmed before editing (reproduced pre-fix, green post-fix) and proven deterministic under a forced skew (`TZ=Pacific/Honolulu`, UTC−10) plus green across UTC and an AEST-ahead zone. Raised Q137 (sweep other tests for the same naive-`date.today()`-vs-AEST anchoring; `test_a_future_measurement_date_is_refused` is a candidate). Handoff: `closeout.md`.

- **HRV consumption — source-agnostic `/recovery/summary` `hrv` block + Samsung unification into `hrv_readings` (`#265`/`#266`, PR #151), landed + merged** - resolves Q130: stored HRV is now read, not just stored. Additive `hrv` block on `GET /recovery/summary` from `recovery_reads.canonical_hrv` (latest canonical night, weekly trend, baseline mean/sd/n); the device-shaped `samsung`/`health_connect`/`has_data` keys are left byte-for-byte. Samsung unified into `hrv_readings` (`source='samsung'`) two ways: the scraper dual-writes each `passive_overnight` nightly HRV via `_upsert_hrv_day`, and a held-then-released insert-only backfill migration (`c1d2e3f4a5b6`, `NOT EXISTS`-guarded, `context='passive_overnight'` collapse — no DDL) migrates history. Read-time union rejected (would make the abstracted helper touch a device table). Operator merge-authorised; the deploy migration-run + live parity (Samsung rows mirrored) is the operator's post-merge verification. Follow-ons: Q134 (full `/summary` restructure + `has_data`), Q135 (drop `samsung_hrv_readings.hrv_ms`), Q136 (model constraint drift). Handoff: `closeout.md`.

- **Garmin HRV historical backfill from the account data export (`#264`, PR #149), landed + merged** - `get_hrv_range` serves only a ~7-day window, so HRV before the live-sync floor is unreachable online; `scripts/garmin_backfill.py` loads nightly HRV from a Garmin account data export (`*healthStatusData.json`, `metrics[type="HRV"].value` + baseline limits) into `hrv_readings`, reusing the connector's RMSSD bounds guard, `_upsert_hrv_day`, and `SessionLocal`. INSERT-ONLY + skip-existing is the safety contract — a night already present is skipped, never updated, so a live-synced night's 5-min series can't be wiped; nightly-only (`hrv_samples` stays a live-sync artifact), `status`/`weekly_avg` left NULL (export vocabulary differs from the live API's), `0.0` baselines → NULL. No schema change (`hrv_readings` pre-exists) — no migration. Reusable operator tool; the live prod run is the operator's out-of-band step, not Code's. Handoff: `closeout.md`.
