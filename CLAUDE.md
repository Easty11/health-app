...
- **Railway prod verification 2026-08-16 closes Q13/Q15/Q18 (#215)** - three OWED verifications, all blocked on the same unreachable surface (prod Postgres) and aged 34-38 days for that one reason, ran read-only in a single session: prod schema is at head (the `3497ab483935` drift was local-behind-prod), the full 15-field bounds sweep over 56 rows found zero violators (discharging `fix/hrv-sleep-integrity` Task 3 with no backfill), and HRV is confirmed absent AT SOURCE rather than unmapped, which eliminates Q5's hypothesis for HRV and transfers the scraper single-point-of-failure to HCA issue #9. Zero prod writes. `FEEDBACK` gains §29 - the Railway dashboard editor silently no-ops a multi-statement paste, which nearly produced a false-negative Q18 close. See DECISIONS_LOG #215.
