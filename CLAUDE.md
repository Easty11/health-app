...
- **Hevy routine read-back — MCP read surface (Q144) — the model can now SEE routines, not just create them. Two live-read, text-output tools mirroring `get_hevy_workouts` (no persistence): `search_hevy_routines` (compact list — title/id/folder/exercise titles + count, no set detail — the cheap existence check a future create-guard reads) and `get_hevy_routine(id)` (full config incl. superset grouping, rest, notes, every set — the made-correctly check). Adds connector `get_routine(id)`; exercise names render as titles (payload-first, template-store fallback). Granularity split [fork] operator-leaned, prior-art-validated (`chrisdoc/hevy-mcp`). Two create-hardening findings logged for the next arc item, not acted on here (Q144: no-delete → check-exists-before-create; routines ignore `rpe` → strip on create). Code, merges on green (`#285`)** - Handoff: `closeout.md`.
