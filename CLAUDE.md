...
- **Visuals increment 2 (v1 test 1 "See" → MET) — `FormChart` (fitness/fatigue/form from the existing `/series/load`, negative-form domain, shared lifted range) + `GET /series/readiness` + observed-readiness `ReadinessChart` (1–5 self-report + passive HRV small multiples) on `/metrics`; actual-vs-forecast dropped to Q141 (applies #277's pattern, no new decision), read-only non-schema, self-merges on green (PR #187)** - Handoff: `closeout.md`.
