...
- **Region→exercise consumer — loose mode-router (#290) — built the prescribe-direction the engine lacked: `select_next` names a region×capacity×side, never a concrete exercise (the model improvised it — the exercise-duplication / 55-routine-sprawl source). New `backend/engine/region_exercise.py`, pure-downstream of `select_next` (prescription untouched), routes each region on a TAGS-AND-MEASURE axis — has `exercise_region_tags` → TRAIN (concrete exercise by region_key+side, laterality-filtered, rotation/recency-avoidance); measure but no tags → SCREEN via the Measure layer (§E hop/decel/COD); neither → honest "no instrument yet" stub (§G); non-region fortify target → honest. Tiered tags: `human_confirmed` trusted, `llm_proposed` usable-but-provisional with the flag SURFACED to the user, none → fallback — ships live off the proposed reference, degrades toward trusted as tags confirm. Loose, not lifecycle (no auto-promotion, no tiering — Q148). REV-2: the brief's "§E is queue_eligible=False" premise was FALSE (only §G is) — Code verified against the tree, Luke ratified the corrected axis before build. Wired into `GET /engine/next`. Code, self-merges on green (`#290`)** - Handoff: `closeout.md`.
