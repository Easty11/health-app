...
- **Schedule occupancy + temporal anchoring (verify-then-fix, decompression-schedule session) — `schedule_item` day-overlap refined to day AND clock-time overlap (`_schedule_overlap_check`/`_times_overlap`): non-overlapping same-day times (work-morning, gym-16:30) now coexist automatically, #233's day-alone trigger superseded but its anti-duplicate guarantee kept (unresolvable time still collides). WS2 mechanism REFUTED-and-recorded: `active` is a stored column + the real retirement lever (top-level `active:false`), NOT `season_end` (inert by design), and the clash already excludes retired rows — fix was discoverability (unknown-field refusal redirects `active` + advertises `distinct_from`; STEP-2 prompt teaches deactivation). WS3: every MCP tool output stamped with a Brisbane `as_of`. WS4 + per-turn re-anchor flagged out-of-repo (Q143). Code + governance, self-merged on green (`#281`)** - Handoff: `closeout.md`.
