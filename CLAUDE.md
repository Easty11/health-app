...
- **Banister stocks seed at the first-week mean load (banister-v4, #304). `load_metrics.compute_window_series` initialises both stocks at `mean(daily_load over the first ACUTE_DAYS(7) calendar days)`, rest days as 0; day 0 IS the seed so `form(0)=0` by construction, recurrence from day 1. Removes the zero-seed warm-up artefact (banister-v3's 0-seed filled the slow fitness stock τ=42 far slower than fatigue τ=10, so form read strongly negative for ~2.5τ regardless of training — user 1 mechanical mean form −3019 May→+234 Aug, maturity dot 'ok' inside the artefact). ACUTE_DAYS reused (no new prior); steady state, τ-set, ΔLoad, 42-day maturity all untouched — a stock recompute. `METRICS_VERSION` v3→v4; `mcp_server._METAB_METRICS_VERSION`→v4. Seed-semantics adjudication (brief S2a ⟂ S3) resolved to the ratified S3 (day 0 = seed, form(0)=0 exactly), reported at G1. banister-v3 rows dormant. Nightly sweep writes v4 via the default version (no operator step, no schema). Backend + tests only; non-migration. Code, self-merged on green (`#304`)** - Handoff: `closeout.md`.
