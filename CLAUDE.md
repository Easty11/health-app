...
- **Chat context renders the resolver POSITION, completing #307 Amendment 1 A2 (#308). #307's S6 had shipped only the DECLARED quota (`_conditioning_line`, off the verbatim microcycle) — no `resolve()`, no done/quota, no due, no uncounted, nothing for capacity slots; that narrowing was reported as "a bounded shape" rather than flagged as a divergence (FEEDBACK §44). `CurrentState.resolver_position = resolve(db, uid, today=today)` — the SAME read the panel uses; a resolver failure is caught+logged+None so the chat still builds (G4). `_section_training_phase(phase, resolver_position)` renders, under the phase: the window line, each slot `{Label} · {done}/{quota}` across BOTH kinds with `◀ DUE` on the `due_slot` match (never `due_capacity`), counted conditioning sessions (sport/duration, `(unzoned)` when trimp 0), the four uncounted reasons in words, an all-met line, and one instruction sentence (authoritative count; don't infer from workout history; flag intent≠quota). `_conditioning_line` folded out; reports resolver numbers verbatim, no recompute. §18 mutation exercised (swap `due_slot`→`due_capacity` → conditioning-due test fails). Two wording corrections carried: HC-exercise ingest gap ≠ Q154 (that is Polar ingest automation; it is #189's unbuilt Status, own Q# from the stage-1 ingest brief); count is not "Polar-Flow-export only" — `polar_v4` `aerobic_sessions` count too. Backend + tests; non-schema, read-only over `resolve()`. Code, self-merged on green (`#308`)** - Handoff: `closeout.md`.
