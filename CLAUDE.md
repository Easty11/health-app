...
- **Accept gated on decision class, with #214's restating confirm in front of the mint (#218)** - an insufficiency-hold is not an acceptable event: it mints nothing, resets nothing, and renders information-only, refused server-side with a 409 rather than merely hidden in the UI. The engine gained a structural discriminator (`CycleDecision.sufficient`, minted at the sufficiency gate and threaded through `replay()` onto `CBTIEvaluationOut`) so an irreversible write's refusal no longer rests on a reason-string prefix. A converged HOLD stays acceptable - it is a decision-on-merits and the block's only record of the level it found, so the verb-level rule "a HOLD is not acceptable" would have been wrong in two cases of three. Accept now runs a two-step confirm restating the actual write, clock reset included. Resolves Q101; selection stays `complete[-1]`; rx 12 let stand. Backend 869->877, frontend 32->41 (the repo's first component tests). See DECISIONS_LOG #218.
