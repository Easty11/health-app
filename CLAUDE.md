...
- **HRV staleness — same-wake-day recency gate + check-in HRV denorm dropped (#327).** `hrv_deviation` admits a source only with a reading ON `for_date` (`stale_sources` surfaced; a dead source's frozen mature baseline no longer speaks for today); MCP readiness keys on the AEST wake-day; Save stops writing `passive_hrv_ms` (retained read-only); prefill + coach daily-record read `select_wakeday_hrv` live; history reads `hrv_readings` by day with a denorm fallback; no "Ring HRV" copy. S5 data correction held separately (`fix/hrv-forward-carry-null`). Code, self-merged on green (`#254`) - Handoff: `closeout.md`.
