...
- **Load chain refreshed by a scheduled orchestrator, not per-sync (#296) — the four independently-rotting manual load modules replaced by one ordered per-user orchestrator (`scripts/refresh_load.py`: Hevy ingest → `load_events` tier0 → `load_events_metabolic` → `load_metrics` ×2) on a nightly dedicated Railway cron service (`backend/railway.cron.toml`, 02:00 AEST), per-user isolated; aerobic-ingest automation deferred (Q154). Code, self-merged on green (`#296`)** - Handoff: `closeout.md`.
