...
- **HRV settling protection activated + 7-value confidence contract ratified (#293/#294/#295) — the deferred wiring half of #292: the settling flag (`hrv_deviation` caps confidence and sets `baseline_state="settling"` for `SETTLING_NIGHTS` after a regime change) shipped unwired, so the metric cried wolf mid-deload. Now the active training-phase change date (`current_training_phase(...).entered_on`) feeds `phase_change_date` in every CONFIDENCE-SURFACING consumer — MCP readiness and `current_state`; scalar-only consumers (`passive_hrv_ms` snapshot, prefill `vs_baseline`) are NOT wired (no-op there). None-safe: no open phase → settling off → prior behaviour; effective only when the regime is a recorded `training_phases` row (#294). `current_state` expanded to surface `baseline_state` on `HRVBaseline` (was scalar-only — the wire would otherwise be invisible), rendered as a low-confidence caveat in `context_builder._section_samsung_hrv`; internal read-model only, no API shape change (#295). The #292 seven-value confidence set (high/medium/medium_low/low/very_low/conflicted/flat) ratified CANONICAL — `flat` confident-neutral, `medium_low` single-mature-source, neither folds into high/medium; brief's five-value enum superseded; docstring provisional framing removed (no behavioural change); threshold constants stay calibration-gated, distinct (#293). Code, self-merges on green (`#293`/`#294`/`#295`)** - Handoff: `closeout.md`.

- **HRV deviation + confidence reader — supersedes `_SOURCE_RANK` arbitration (#292) — the correct multi-source consumption model, superseding #291's arbitrated scalar: `reads.recovery_reads.hrv_deviation` normalises each source to its OWN rolling baseline (z-score), combines the per-source deviations by weight, and surfaces cross-source disagreement as CONFIDENCE — never blends raw ms (the offset is non-constant, 0–48 ms across 5 dual-wear nights) and never picks one arbitrated winner. Two devices on one night are two instruments, not two candidates for one truth. All four #291 consumers migrated ALL-OR-NOTHING (`_snapshot_passive`/`passive_hrv_ms`, prefill `vs_baseline`, `current_state.hrv_baseline`, MCP readiness figure), each taking the full object or a documented `representative_source` (highest-weight) number so no surface contradicts another. A thin just-connected source contributes at low weight until mature — it cannot flip an established source on day one. Seven thresholds are named, parameterised constants, PROVISIONAL (n=5-deload) and calibration-gated on ~3–4 wk representative-load dual-wear (AGREEMENT_HIGH/MED first). Dual-reader is a TRANSITION with a defined exit (Q153): delete `_SOURCE_RANK` + the arbitration branch once no consumer calls `.canonical` — only unmounted recovery.py (Q151) still does — keeping the shared `_hrv_rows` plumbing. Code, self-merges on green (`#292`)** - Handoff: `closeout.md`.

- **HRV consumption rewire — live readiness/context surfaces read `canonical_hrv` (Garmin-aware, Q130) — the deferred consumption half of Q130: the AM passive snapshot (`_snapshot_passive` → `daily_records.passive_hrv_ms` → `/series/readiness`), the prefill + `current_state` HRV baseline, and the MCP readiness figure now read the HRV SCALAR through `reads.recovery_reads.canonical_hrv` (source-agnostic over `hrv_readings`, Garmin-arbitrated) instead of `SamsungHRVReading` directly, so Garmin HRV surfaces once connected. Surgical, scalar-only: every sleep/SpO2/architecture field stays on `SamsungHRVReading` (Garmin supplies none); the HRV-only boundary of `hrv_readings` holds — no schema change. `canonical_hrv` gains `as_of` (upper bound `captured_at <= as_of`, applied before arbitration so same-night groups stay complete) for the snapshot's at-or-before-`for_date` read. Device-readout rule [Luke-ratified]: `/health/*`, `context_builder._section_samsung_hrv`, and `mcp_server.get_recovery_metrics` stay wholly Samsung (a readout keeps device attribution — never a Garmin number under a Samsung banner); `recovery.py` stays unmounted (→ Q151). Forward-only — historical `passive_hrv_ms` backfill deferred (Q152); no-op today (canonical == Samsung, per-user overlap nil). Full backend suite 1606 passed. Code, self-merges on green (`#291`)** - Handoff: `closeout.md`.
