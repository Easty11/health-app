...
- **Hevy exercise-action lane folded onto WriteResult (Q144, builds on #286) — the LAST un-wrapped Hevy write surface is closed; the acknowledgement-discipline arc (schedule / `/chat` / Hevy routine / Hevy exercise) is complete. `_process_exercise_actions` now returns per-block `write_results`, paired 1:1 with the action string so a failed/rejected/unconfirmed/already-present custom-exercise create can't be narrated as a fresh success. WRAP-ONLY, no collision guard: unlike routines (#286), the lane is already idempotent (the `resolve_exercise` pre-check + `create_and_resolve`'s #65 pre-check + the #212 freshness gate structurally prevent a duplicate mint), so the conditional [fork] resolved to add nothing. New informational reason_codes `already_present` (saved=False — nothing minted, so a "created!" is corrected to "already in your catalogue", ℹ️ not ✓), `limit_reached`, `invalid_exercise_field`, `created_unconfirmed` (verbatim no-retry guidance rides the action string). Exercise + routine + knowledge results feed the one narrate-after-write pass + footer. Code, self-merges on green (`#288`)** - Handoff: `closeout.md`.

- **Hevy routine-create hardening + superset write (Q144, builds on #285) — the create path is now correct, safe, honest, and capable. Harden (#286): `rpe` stripped from routine set payloads (deterministic connector floor — no workout-create path exists, so load logging is untouched); a `RoutineAlreadyExists` idempotency guard reads the paginated `/routines` list and refuses a same-(title, folder) create before the POST (case-insensitive title + equal folder — [fork] Luke-ratified refuse+offer / title+folder; Hevy has no delete and update replaces contents); `_process_routine_actions` folds onto `WriteResult` (Hevy codes `created`/`already_exists`/`unresolved_exercise`/`invalid_json`/`invalid_shape`/`create_failed`, `already_exists` user_resolvable), assembly feeds routine + knowledge results into the one narrate-after-write pass + footer, REST lane → 409. Extend (#287): `superset_id` + fragile conventions (unilateral-pairing example, `superset_id: 0` ≠ null, ≥1 set, snake_case, omit-weight-not-zero, no rpe, no `@`) documented in `_section_routine_creation` (parity-safe; pinned by `test_routine_creation_prompt.py`). Exercise-action lane deliberately left string-only — the last un-wrapped Hevy write surface. Code, self-merges on green (`#286`/`#287`)** - Handoff: `closeout.md`.
