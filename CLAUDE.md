...
- **Contraindication block sets revised per the Q23 audit (#216)** - `_RADICULAR_BLOCKS` gains the PC-length screen (functionally an SLR, a neural provocation test) and the carry G-axis twin; `_RA_FLARE_BLOCKS` gains grip and the same twin; and the radicular arm is scoped to spinal body parts so a peripheral neural entry no longer triggers a lumbar-shaped stand-down. Protective, not corrective - neither set has ever fired in prod. The live defects the audit surfaced are in `restrictions[]` consumption, spawned as `Q102` and deliberately NOT patched here. First product-code session since #213. See DECISIONS_LOG #216.
