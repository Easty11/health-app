...
- **OPEN_QUESTIONS Q100: user-5 zero-customs investigation (the #211 follow-up chip)** - Cooper's (user 5) stored Hevy key authenticates but the account behind it holds 0 customs AND 0 workouts; a read-only prod probe rules out H1 (user-id mapping — `user_integrations` row id 11 is genuinely Cooper's) and the copied-key form of H2 (key SHA unique vs Luke/Deb), and cannot distinguish stale-belief from wrong-account from our side (Hevy exposes no whoami, 0 workouts = no in-band identity signal), so resolution needs Cooper. ROADMAP NEXT row struck DONE→Q100. Governance/OPEN_QUESTIONS-only; no decision minted. See closeout.md.
- **Catalogue freshness on workout-fetch (Q75 → #211)** - staleness-gated per-user Hevy template sync piggybacked on `GET /hevy/workouts[/all]`, closing the 2026-08-05 stale-catalogue drift (in-session customs invisible to the resolver). Per-user marker `user_integrations.templates_synced_at` (migration `e2d5c7a1b9f3`), rejecting the briefed aggregate-over-`synced_at` premise as multi-user-wrong. First product-code session after three instrument/governance ones. Two follow-ups queued (ROADMAP NEXT + chips): create-time freshness gate, user-5 zero-customs. See DECISIONS_LOG #211.
- **Dispatch brief: §19 ledger row (secret-render env-check) · governance-view regen · interpretation training-wheels verify** - lands FEEDBACK §19.6 ledger row id-20 (expansion-based secret presence checks `${VAR:-}` render the value; presence must be exit-code-based; implementation guidance under #110/#111, no rule minted); regenerates the consolidated governance view against master (#210/Q99 parity, `build/`-only, uncommitted — Luke places it via the claude.ai UI); records interpretation training-wheels review #1 complete (verified in-DB, one `human_verified` row), a production state fact, not a store write. INSTRUMENT/governance session; no product code moved. See closeout.md.
