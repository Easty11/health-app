...
- **Routine-create numeric floor + folder name→id resolution (Concern A) — root-caused the `Expected number, received nan` 400 that failed all three decompression routines. VERIFY disproved the folder-coercion guess (the chat lane reads `folder_id` and drops the model's `folder` name — no coercion anywhere); the real mechanism (reproduced offline): `json.loads` accepts a literal `NaN` token → the builder passed it through → httpx serialized it (`allow_nan=True`) to the wire. Fix closes the CLASS: connector raises typed `RoutineNumericError` (`invalid_number`) naming field+set+exercise before any network call (NaN/±Inf/string/bool rejected, `0` stays valid), plus a `allow_nan=False` serialize backstop so nothing non-finite is ever sent; chat → `invalid_number` WriteResult (system_issue), REST → 422. Folder name→id resolution added (`get_routine_folders` + case-insensitive match); a miss creates unfoldered + `created_unfoldered` (saved=True, no retry) instead of silently dropping the folder. Concern B (schedule read-back) is verification-only: code analysis says duplicate rows are structurally impossible (key-superseding upsert) and the ten-ticks doubling is cosmetic, but a real prod read-back is owed (Q146). Findings logged Q145 (WED corrupted id) / Q147 (numeric floor on the read path?). Code, self-merges on green (`#289`)** - Handoff: `closeout.md`.
---
