...
- **Document the `training_plan` / `satisfies` write shapes + type-aware write footer + strip raw `<thinking>` (#313). #312's S5(iv) instructed the coach to write a `training_plan` and set `satisfies`, but the `<knowledge_update>` write-shape home documented neither (prod-confirmed: a `training_plan` save under a wrong key was correctly refused). Both shapes are now documented in the one write-shape home, field lists GENERATED from `TRAINING_PLAN_FIELDS` / `_SATISFIES_VALIDATORS` (cannot drift); the rewrite instruction states it SUPERSEDES not appends and the macro is stored verbatim. The write-result footer is type-aware (`_entry_noun`) so a `training_plan` refusal is no longer mislabelled "Schedule entry". `_strip_thinking` removes a raw `<thinking>` block from the final reply. Non-schema; FEEDBACK §45 (a prompt sentence that instructs a write must sit beside the schema for that write). Code, self-merged on green (`#313`)** - Handoff: `closeout.md`.
