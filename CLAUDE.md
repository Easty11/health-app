...
- **The weekly template carries capacity quotas, never sport or position labels (#221)** - `fortification_profiles.weekly_template` (migration `b7c3e91f2a48`) holds a capacity-quota microcycle - `{slots: [{capacity, sessions_per_week, minutes}]}`, where `capacity` is a taxonomy `Capacity` member and the shape refuses unknown keys at either level, so a `sport` field is a schema change rather than a convention drift. Sport-specificity stays expressed through `vehicle_bias` / `horizon` / `primary_target`: two users with identical templates and different bias get different programmes, which is the design, not a gap. The refusal is recorded because the temptation to add a `sport` field reads as the obvious missing piece and will recur - a lookup table of sport -> programme is exactly the hardcoded-string architecture the profile replaced. `select_next(..., capacity=)` and `GET /engine/next?capacity=` consume ONE slot's capacity as a SUBSET filter over a queue `compute_probe_queue` has already stop-filtered, which makes "the slot filter never overrides a stop" structural rather than an ordering convention. Absent the parameter, behaviour AND response shape are unchanged. The Fortify target is not filtered by the slot - it is a declared field, not a ranked candidate, so a mismatch is reported (`slot.fortify_target_matches_slot`) rather than silently served or silently dropped. Validated at write and stored verbatim, so `PUT` then `GET` is byte-identical; nothing resolves WHICH slot is due, and how `minutes` reaches the prescription is deliberately unresolved (Q106, with Q105 on the verbatim-storage trade). Backend 890->945, frontend untouched. See DECISIONS_LOG #221.

- **Canonical marker map is DB-backed, with a guarded runtime bind at the confirmation screen (#220)** - the map moves from a startup-loaded `marker_canonical.json` dict to `marker_canonical_entries`, seeded from that JSON (70 rows, migration self-checks the count); `labs.py` loses `_CANONICAL_MAP` and both the confirm resolution and `/canonical-map` read the table per request. `POST /labs/canonical/bind` writes an entry AND promotes that marker's historical unmapped rows, which is what makes a bind worth doing (#159) - ending the hand-edit-JSON-plus-backfill chore run four times (PSA, conjugated bilirubin, CK, Cystatin C). The over-collapse unit-guard is carried to both new points where identity became mutable at runtime, bind-time and backfill-time; a refused bind promotes zero rows. Binding is optional - an unbound row still stores null (#58/#155 retain-raw). #50 is now fully implemented and its stale "Not implemented" status superseded. The two interpretation readers stay on the JSON, PHASED: a bound-ungrouped marker DOES reach `rephrase` (the brief's premise was wrong), but both degrade safe - `_KNOWN_ENTITIES` is a detector allowlist so staleness can only miss a detection, never false-reject; raised as `Q104`. Backend 882->890, frontend 41->47. See DECISIONS_LOG #220.

- **Naps attribute to the night they precede — Q45 closed on operator determination (#219)** - the exclusion logic is REVERSED: `replay.load_nights` now reads Night(W)'s nap from row W-1 and `NAP_EXCLUDE_MIN` rises 0->30, so a nap contaminates the night it came before rather than disqualifying the night it was logged against. The close is the substance — which night a nap belongs to is a modelling convention the operator sets, not a fact needing clinical provenance, so Q45's "confirm from VA protocol or clinician" bar is superseded as the wrong gate; the workbook's scoped-null search stands and is not re-run. The code now honours the contract `models.DailyRecord.naps_min`'s column comment has declared since the column was created and the engine never kept. Reworked from the orphan `fix/q45-nap-attribution` (`4f77679`) by re-derivation, not replay - the orphan predated #218 and three of its four frontend targets had moved or did not exist. Q78 stays OPEN, unblocked rather than resolved. Backend 877->882, frontend 41. See DECISIONS_LOG #219.
