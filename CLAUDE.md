...
- **Garmin auth: curl_cffi-era garminconnect 0.3.11, garth dropped (`#263`, PR #147), landed + merged** - the pre-curl_cffi garth auth was broken server-side by Garmin's March-2026 Cloudflare TLS fingerprinting (new logins fail), so the "pin EXACT" froze the breakage; moved `garminconnect` 0.3.2→0.3.11 (rebuilt on curl_cffi — Chrome JA3 TLS impersonation — and garth-free) and removed `garth` from requirements (curl_cffi arrives transitively; nothing imports garth). Dependency + doc-only: the token/exception surface (`login(inline)`, `client.dumps()`, `prompt_mfa`/`return_on_mfa`, `GarminConnectAuthenticationError`) is unchanged across 0.3.2→0.3.11, so no connector logic changed. Requires Python ≥3.12 (prod builds on 3.12 — verified live); deploy `18f0db72` SUCCESS with 0.3.11 + curl_cffi 0.16.2, garth gone. Live-login proof is the operator's post-merge step. Resolves Q133; supersedes the #259 pin. Handoff: `closeout.md`.
