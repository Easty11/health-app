...
- **Structured save-result contract (Q143a / WS4) — `ChatResponse.write_results` now carries one machine-checkable `{saved, reason_code, reason, key}` per `<knowledge_update>` block (per-row, never a roll-up), paired 1:1 with the `actions_taken` prose so a failed write can't be narrated as success. Type-derived `reason_code` vocab (`saved`/`deactivated`/`not_found`/`day_time_clash`/`unknown_field`/`invalid_shape`/`invalid_json`/`error`; `ScheduleItemInvalid(ValueError)` carries `unknown_field`). Verified: the write surface is the CHAT channel not an MCP tool (all 7 are reads), `POST /knowledge/entry` was already structured. Substrate only — the consuming gate has NO in-repo target (no chat consumer in `frontend/`; lives in the companion/agent layer, Q143). Code, self-merged on green (`#283`)** - Handoff: `closeout.md`.
