...
- **v4 zone-enrichment folded into the Polar sync (`#261`, PR #144), landed + merged** - the v4 summary transport omits the HR-zone split, so live-synced `polar_v4` rows land zoneless; the sync now runs a second feature-mode pass (`features=zones`, one-day window cap — live-probe-confirmed) that fetches per-exercise zones for the window's zoneless-row dates and merges `z*_seconds` in place by `source_session_id`, and the on-ingest cascade recomputes `metab-v1` (no formula bump). flow_export still outranks v4 (#260), so a dual-lane bout stays single-emission. Enriches the sync window only — a one-time wide-window sync backfills history. Handoff: `closeout.md`.
