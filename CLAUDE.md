...
- **Dated one-offs documented for the coach (§45) · plan-of-record on the Phase card · advanced phase link (#319, follow-up). (1) §45 applied: STEP 2's `schedule_item` write-shape docs (`context_builder._section_user_profile`) now document `event_date`/`event_end` with a worked example + the routing rule (a KNOWN-DATE commitment → `schedule_item` with `event_date`, NOT `load_context`; a vague undated state stays `load_context`), and the stored-field list is GENERATED from `SCHEDULE_ITEM_FIELDS` (the #313 drift-proof pattern) so the #317 undocumented-field gap cannot recur; all edits inside the STEP1→STEP3 parity-excised span. (2) Plan of record on the card: ONE STALE definition `context_builder.plan_of_record_stale` (extracted from `_section_training_plan`, which delegates byte-identically) + `GET /engine/plan-of-record` → `{macro, revised_on, revised_by, stale}`; `PhaseCard` renders the headline + STALE badge (full macro on expand), never re-deriving the flag client-side — closing the #318 §44 divergence. (3) `PhaseForm`/`ClosePhaseDialog` kept (not deleted): an "advanced · open / close phase directly" disclosure on the card mounts them until the 8-step flow is prod-confirmed. GUARD: no change to `resolve()`/validators/#275; pure extraction, read-only route, frontend. Backend + frontend + tests; non-schema. Code, self-merged on green (`#319`)** - Handoff: `closeout.md`.

- **Phase card + structured 8-step phase-change flow (PR2 frontend) (#318). `PhaseCard` is the phase surface — current phase + week N + review badge, the quota position (`QuotaWindow`), the #312/#316 week line (scheduled·quota·done + freshness, from `GET /engine/week-plan`), history, and ONE action "Review / change phase" — replacing ExposurePanel's ad-hoc Open/Close buttons + inline PhaseForm/ClosePhaseDialog (a phase change is the form's single confirmed atomic write now). `PhaseTransitionFlow` is the 8 steps: review the outgoing phase (per-leg done/quota) + verdict/Continue-or-Move → the new phase (label locked on Continue) → dated one-off events → quota slots → the microcycle (no JSON typing; `device_sports` from `sport_names_seen`; a JSON escape hatch shows the build) → placement (linked `schedule_item`s + a live scheduled-vs-quota echo + an acknowledgement tick that gates Next on EXCESS/UNPLACED) → `recorded_via` per slot (WARN, never block, when no evidence path) → Hevy folder → confirm, one button, POST `/engine/phase/transition`. Server is the validator (#230): a 422 `detail` is shown VERBATIM, never re-validated client-side; the draft lives in component state only (leaving discards it). Divergence (§44): the plan-of-record headline is OMITTED from the card (already in the chat context #312/#313; adding it needs a new read or a duplicated STALE derivation). PhaseForm/ClosePhaseDialog orphaned, retained as a cleanup candidate. GUARD: no coach write path to phases, no auto-open on review_on, the form never parses plan prose; frontend only, no backend change. Frontend suite 203 passed; served-bundle grep (#121) operator-side. Code, self-merged on green (`#318`)** - Handoff: `closeout.md`.

- **Phase-change transition (PR1 backend) + dated one-off schedule items + `recorded_via` (#317). `POST /engine/phase/transition` performs ONE atomic write behind the phase-change FORM (the operator's ruling: a form, not a coach chat): close the outgoing phase + open the new one (shared non-committing `_apply_open_phase`, same validator, no fork), upsert/retire the listed schedule items (shared `_stage_upsert_entry`), merge `phase_folders` — one commit; any failure rolls the whole set back, nothing written (G1). The Hevy folder create is the ONLY external call and runs FIRST outside the txn (abort clean on fail; orphan named on txn fail). Idempotency is natural-state, cross-worker, non-schema: a re-submit deep-equal to the OPEN phase (label+entered_on+microcycle) returns it 200 no_op, no second folder. Same-day correction is PERMITTED (the monotonic rule refuses only `entered_on < prior`, strict — reported, unchanged). `schedule_item.event_date`/`event_end` (closes Q165 — a dated one-off, exclusive with `days`; `plan_week` marks the days unavailable + heavy→next-day caution + ignore-when-past; `_section_schedule` dated line). `recorded_via` on microcycle slots (optional closed set, form-only metadata, so the form can WARN when a slot has no evidence path). `GET /engine/phase/transition/draft`: outgoing per-leg review BOUNDED to ≥ entered_on (never inherits resolve()'s pre-entry extrapolation), items, `sport_names_seen` (new allow-listed door read), freshness, best-effort folders. Coach points to the Phase card, never writes a phase/microcycle/phase_folders. GUARD: no change to resolve() or the phase/microcycle/satisfies validator RULES or #275; the form never parses plan prose. Backend + tests; non-schema. `Q165 → DONE`; `Q166` raised (a structured planned-phase store for "Move" prefill, not built). PR2 (Phase card + 8-step flow) is the next lane (#318). Code, self-merged on green (`#317`)** - Handoff: `closeout.md`.
