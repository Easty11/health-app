...
# Snapshots live OUTSIDE both repos (DECISIONS_LOG #N+2): a cross-repo view inside one repo
...
# --------------------------------------------------------------------------------------
# Snapshot writer  (DECISIONS_LOG #N+2 / #N+3)
# --------------------------------------------------------------------------------------
...
repos on purpose (DECISIONS_LOG #N+2): a cross-repo view inside one repo recreates a
...
def write_snapshot(model: dict, out_dir: Path) -> tuple[Path, bool]:
    ...
    is the baseline (#N+2/#N+3). Provenance per repo is already carried in the model.
    """
