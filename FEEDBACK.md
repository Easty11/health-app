...
- §40 A chart brief must VERIFY the scale, unit, and population of every series BEFORE the chart is specced — rendering tests prove geometry (line count, dots, domain) and can never catch a comparison that is meaningless or a series that is empty. Two briefs in a row specced a picture that would have rendered fine and meant nothing: increment 1 overlaid load windows on different units (`kg_reps` vs `nm_au`) on one axis (resolved to per-window / small-multiple), and increment 2 specced `morning_readiness` (1–5 ordinal, "primary OUTCOME") against `model_forecast` (0–10, written nowhere) as a residual — a subtraction across scales, over a column that is null in prod (Q141). Extends the units concern: before the endpoint shape is fixed, confirm for each series (a) its scale and unit, (b) that any two series drawn together share them or get separate axes, and (c) that the field is actually populated — a green vitest render is not evidence the chart says anything true
